...
static uint8_t *read_file(const char *path, size_t *len_out) {
  ...
  if (!f) { fprintf(stderr, "mkwad: cannot open %s\n", path); return NULL; }
  fseek(f, 0, SEEK_END); long n = ftell(f); fseek(f, 0, SEEK_SET);
  if (n < 0) { fclose(f); return NULL; }
  uint8_t *buf = malloc((size_t)n ? (size_t)n : 1);
  if (buf && fread(buf, 1, (size_t)n, f) != (size_t)n) { free(buf); buf = NULL; }
  fclose(f);
  if (!buf) { fprintf(stderr, "mkwad: read failed %s\n", path); return NULL; }
  *len_out = (size_t)n;
  return buf;
}
...
static int load_common_key(void)
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (v < 0) {
      fprintf(stderr,
        "mkwad: %s: unexpected character '%c' (0x%02x) at offset %zu\n",
        path, isprint(c) ? c : '?', (unsigned)c & 0xff, i);
      return -1;
    }
    if (nibbles >= 32) {
    ...
    if ((nibbles & 1) == 0) k[nibbles / 2]  = (uint8_t)(v << 4);
    else                    k[nibbles / 2] |= (uint8_t)v;
    nibbles++;
  }
  if (nibbles != 32) {
    fprintf(stderr,
      "mkwad: %s: expected 32 hex digits or a 16-byte raw file, got %d nibbles\n",
      path, nibbles);
    return -1;
  }
...
static void build_ticket(uint8_t tik[TICKET_SIZE], uint64_t title_id,
                         const uint8_t enc_title_key[16]) {
...
typedef struct { uint32_t cid; uint16_t index; uint16_t type; uint64_t size; uint8_t sha1[20]; } content_rec_t;

static uint8_t *build_tmd(uint64_t title_id, uint32_t ios, uint16_t region,
                          uint16_t title_version, uint16_t boot_index,
                          const content_rec_t *recs, int n, size_t *out_len) {
...
static uint8_t *encrypt_content(const uint8_t *plain, size_t plain_len,
                                uint16_t index, content_rec_t *rec,
                                size_t *enc_len_out) {
  ...
  uint8_t iv[16]; memset(iv, 0, 16); put_be16(iv, index);
  aes128_cbc_encrypt(KOS_TITLE_KEY, iv, buf, enc_len);
  rec->index = index; rec->type = 1; rec->size = plain_len;
  *enc_len_out = enc_len;
  return buf;
}
...
static int write_wad(const char *out_path, const uint8_t *tik,
                     const uint8_t *tmd, size_t tmd_len,
  ...
  size_t data_sz = 0;
  for (int i = 0; i < n; i++) data_sz = ALIGN_UP(data_sz, 0x40) + enc_len[i];
  ...
  if (!f) { fprintf(stderr, "mkwad: cannot write %s\n", out_path); free(w); return -1; }
  size_t wr = fwrite(w, 1, total, f);
  fclose(f);
  free(w);
  if (wr != total) { fprintf(stderr, "mkwad: short write\n"); return -1; }
...
static void wad_usage(void) {
  ...
  printf("  --title-id <4ch>  4-char low title id (default KOSL); high = 00010001\n");
  printf("  --ios <N>         Required IOS / system version (default 58)\n");
  printf("  --title <name>    Channel name shown in the System Menu (default wii-load-ip)\n");
  printf("  --region <0-3>    0=JP 1=US 2=EU 3=free (default 3)\n");
  printf("  --banner <file>   Use a prebuilt banner content instead of the generated one\n");
  printf("  --sound <file>    Add a prebuilt meta/sound.bin (IMD5+BNS) to the generated banner\n\n");
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    const char *a = argv[i];
    if      (!strcmp(a, "--dol")     && i+1 < argc) dol_path = argv[++i];
    else if ((!strcmp(a, "-o") || !strcmp(a, "--out")) && i+1 < argc) out_path = argv[++i];
    else if (!strcmp(a, "--title-id") && i+1 < argc) { strncpy(tid4, argv[++i], 4); tid4[4]=0; }
    else if (!strcmp(a, "--ios")     && i+1 < argc) ios = (uint32_t)strtoul(argv[++i], NULL, 0);
    else if (!strcmp(a, "--title")   && i+1 < argc) title = argv[++i];
    else if (!strcmp(a, "--region")  && i+1 < argc) region = (uint16_t)strtoul(argv[++i], NULL, 0);
    else if (!strcmp(a, "--title-ver") && i+1 < argc) title_version = (uint16_t)strtoul(argv[++i], NULL, 0);
    else if (!strcmp(a, "--banner")  && i+1 < argc) banner_path = argv[++i];
    else if (!strcmp(a, "--sound")   && i+1 < argc) sound_path = argv[++i];
    else if (!strcmp(a, "-h") || !strcmp(a, "--help")) { wad_usage(); return 0; }
    else { fprintf(stderr, "mkwad: unknown option %s\n", a); wad_usage(); return 1; }
  }
  if (!dol_path || !out_path) { wad_usage(); return 1; }
  ...
  if (load_common_key() < 0) return 1;

  uint64_t title_id = 0x0001000100000000ull |
      ((uint64_t)(uint8_t)tid4[0] << 24) | ((uint64_t)(uint8_t)tid4[1] << 16) |
      ((uint64_t)(uint8_t)tid4[2] << 8)  |  (uint64_t)(uint8_t)tid4[3];
  ...
  if (!banner || !dol) { free(banner); free(dol); return 1; }

  content_rec_t recs[2];
  size_t enc_len[2];
  uint8_t *enc[2];
  recs[0].cid = 0; enc[0] = encrypt_content(banner, banner_len, 0, &recs[0], &enc_len[0]);
  recs[1].cid = 1; enc[1] = encrypt_content(dol,    dol_len,    1, &recs[1], &enc_len[1]);
  free(banner); free(dol);
  if (!enc[0] || !enc[1]) { free(enc[0]); free(enc[1]); return 1; }

  /* wrap title key under the common key (IV = title id || 0) */
  uint8_t enc_tk[16]; memcpy(enc_tk, KOS_TITLE_KEY, 16);
  uint8_t iv_tk[16]; memset(iv_tk, 0, 16); put_be64(iv_tk, title_id);
  ...
  size_t tmd_len = 0;
  uint8_t *tmd = build_tmd(title_id, ios, region, title_version, /*boot_index*/1,
                           recs, 2, &tmd_len);
  if (!tmd) { free(enc[0]); free(enc[1]); return 1; }

  int rc = write_wad(out_path, tik, tmd, tmd_len, enc, enc_len, 2);

  free(tmd); free(enc[0]); free(enc[1]);
  if (rc == 0)
