...
static void iso_lite_set_error(char *error_buf, size_t error_buf_size,
                               const char *fmt, ...)
{
    va_list ap;
...
static void iso_lite_write_padded_field(uint8_t *dst, size_t width,
                                        const char *value)
{
    size_t i;
...
static uint8_t iso_lite_write_dir_record(uint8_t *dst, uint32_t extent_lba,
...
static void iso_lite_write_validation_entry(uint8_t *dst)
{
    ...
    for (i = 0; i < 16; i++) {
        ...
        checksum = (uint16_t)(checksum +
                   ((uint16_t)dst[i * 2] | ((uint16_t)dst[i * 2 + 1] << 8)));
    }
...
static void iso_lite_write_boot_entry(uint8_t *dst, uint32_t boot_lba,
                                      uint16_t boot_load_sectors)
{
    dst[0] = 0x88;
    dst[1] = 0x00;
...
void iso_lite_normalize_file_name(const char *input, char *output, size_t output_size)
{
    ...
    if (dot && dot != name) {
        for (i = 0; i < 8 && name[i] && &name[i] < dot; i++) {
            char c = name[i];
            base[i] = (char)(isalnum((unsigned char)c) || c == '_' ?
                             toupper((unsigned char)c) : '_');
        }

        for (j = 0; j < 3 && dot[1 + j]; j++) {
            char c = dot[1 + j];
            ext[j] = (char)(isalnum((unsigned char)c) || c == '_' ?
                            toupper((unsigned char)c) : '_');
        }
    } else {
                          ...
                          size_t error_buf_size)
{
    ...
    if (!output || !config || !files || !file_count) {
        iso_lite_set_error(error_buf, error_buf_size,
                           "mkiso-lite requires at least one input file");
        return -1;
    }
    ...
    for (i = 0; i < file_count; i++) {
        if (!files[i].iso_name || !files[i].iso_name[0]) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
                               "input file %zu is missing an ISO name", i);
            return -1;
        }
        ...
        if (files[i].size > UINT32_MAX) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
                               "input file %s is too large", files[i].iso_name);
            return -1;
        }
    ...
    if (root_bytes > ISO_LITE_SECTOR_SIZE) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "root directory would exceed one ISO sector");
        return -1;
    }
    ...
    if (use_eltorito) {
        for (i = 0; i < file_count; i++) {
            if (strcmp(layout[i].iso_name, boot_name) == 0) {
                ...
                boot_load_sectors = (uint16_t)
                    (((layout[i].size + ISO_LITE_SECTOR_SIZE - 1) /
                      ISO_LITE_SECTOR_SIZE) * 4U);
                break;
            }
        }
        ...
        if (!boot_image_lba || !boot_load_sectors) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
                               "El Torito boot file %s was not found",
                               boot_name);
            return -1;
        }
    }
    ...
    if (config->system_area_size > 16U * ISO_LITE_SECTOR_SIZE) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "system area is larger than 16 ISO sectors");
        return -1;
    }

    if (config->system_area && config->system_area_size) {
        if (fwrite(config->system_area, 1, config->system_area_size, output) !=
            config->system_area_size) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
    ...
    if (iso_lite_write_zeros(output,
                             16U * ISO_LITE_SECTOR_SIZE - config->system_area_size) < 0) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to pad system area");
        return -1;
    }
    ...
    iso_lite_write_dir_record(pvd + 156, root_dir_lba + config->start_lba,
                              ISO_LITE_SECTOR_SIZE, 0x02, root_name, 1);
    ...
    if (fwrite(pvd, 1, sizeof(pvd), output) != sizeof(pvd)) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to write primary volume descriptor");
        return -1;
    }
    ...
    if (use_eltorito) {
        ...
        iso_lite_write_le32(boot_record + 71,
                            boot_catalog_lba + config->start_lba);

        if (fwrite(boot_record, 1, sizeof(boot_record), output) != sizeof(boot_record)) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
                               "failed to write boot descriptor");
            return -1;
        }
    ...
    if (fwrite(terminator, 1, sizeof(terminator), output) != sizeof(terminator)) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to write volume descriptor terminator");
        return -1;
    }
    ...
    if (current_lba < path_table_l_lba &&
        iso_lite_write_zeros(output,
                             (size_t)(path_table_l_lba - current_lba) *
                             ISO_LITE_SECTOR_SIZE) < 0) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to pad before path table");
        return -1;
    }
    ...
    if (fwrite(path_table_l, 1, sizeof(path_table_l), output) != sizeof(path_table_l)) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to write little-endian path table");
        return -1;
    }
    ...
    if (current_lba < path_table_m_lba &&
        iso_lite_write_zeros(output,
                             (size_t)(path_table_m_lba - current_lba) *
                             ISO_LITE_SECTOR_SIZE) < 0) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to pad before big-endian path table");
        return -1;
    }
    ...
    if (fwrite(path_table_m, 1, sizeof(path_table_m), output) != sizeof(path_table_m)) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to write big-endian path table");
        return -1;
    }
    ...
    if (current_lba < root_dir_lba &&
        iso_lite_write_zeros(output,
                             (size_t)(root_dir_lba - current_lba) *
                             ISO_LITE_SECTOR_SIZE) < 0) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to pad before root directory");
        return -1;
    }
    ...
    lba = 0;
    lba += iso_lite_write_dir_record(root_dir + lba, root_dir_lba + config->start_lba,
                                     ISO_LITE_SECTOR_SIZE, 0x02, root_name, 1);
    lba += iso_lite_write_dir_record(root_dir + lba, root_dir_lba + config->start_lba,
                                     ISO_LITE_SECTOR_SIZE, 0x02, parent_name, 1);

    if (use_eltorito) {
        static const uint8_t boot_cat_name[] = "BOOT.CAT;1";
        lba += iso_lite_write_dir_record(root_dir + lba,
                                         boot_catalog_lba + config->start_lba,
                                         ISO_LITE_SECTOR_SIZE, 0x00,
                                         boot_cat_name,
    ...
    for (i = 0; i < file_count; i++) {
        ...
        lba += iso_lite_write_dir_record(root_dir + lba,
                                         layout[i].extent_lba + config->start_lba,
                                         layout[i].size, 0x00,
                                         (const uint8_t *)layout[i].iso_name,
                                         name_length);
    }
    ...
    if (use_eltorito) {
        if (current_lba < boot_catalog_lba &&
            iso_lite_write_zeros(output,
                                 (size_t)(boot_catalog_lba - current_lba) *
                                 ISO_LITE_SECTOR_SIZE) < 0) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
                               "failed to pad before boot catalog");
            return -1;
        }
        ...
        iso_lite_write_boot_entry(boot_catalog + 0x20, boot_image_lba,
                                  boot_load_sectors);

        if (fwrite(boot_catalog, 1, sizeof(boot_catalog), output) != sizeof(boot_catalog)) {
            free(layout);
            iso_lite_set_error(error_buf, error_buf_size,
                               "failed to write boot catalog");
            return -1;
        }
    ...
    if (current_lba < layout[0].extent_lba &&
        iso_lite_write_zeros(output,
                             (size_t)(layout[0].extent_lba - current_lba) *
                             ISO_LITE_SECTOR_SIZE) < 0) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to pad before file data");
        return -1;
    }
    ...
    if (trailing_pad_sectors &&
        iso_lite_write_zeros(output,
                             (size_t)trailing_pad_sectors * ISO_LITE_SECTOR_SIZE) < 0) {
        free(layout);
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to write trailing pad sectors");
        return -1;
    }
                         ...
                         size_t error_buf_size)
{
    ...
    if (!output) {
        iso_lite_set_error(error_buf, error_buf_size,
                           "failed to open %s for writing", output_path);
        return -1;
    }

    rc = iso_lite_write_stream(output, config, files, file_count, stats,
