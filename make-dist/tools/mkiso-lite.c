...
static void mkiso_set_path_error(char *error_buf, size_t error_buf_size, const char *prefix,
                                 const char *path) {
    size_t prefix_len;
    size_t available;
    size_t path_len;
...
static int mkiso_add_file(const char *full_path, const char *entry_name, mkiso_input_file_t **files_out,
                          size_t *count_out, size_t *capacity_out, char *error_buf, size_t error_buf_size) {
    ...
    if(count == capacity) {
        ...
        mkiso_input_file_t *new_files = realloc(files, new_capacity * sizeof(*files));
        if(!new_files) {
    ...
    if(mkiso_load_file(full_path, &data, &size) < 0) {
        mkiso_set_path_error(error_buf, error_buf_size, "failed to read ", full_path);
        return -1;
    }

    memset(&files[count], 0, sizeof(files[count]));
    iso_lite_normalize_file_name(entry_name, files[count].iso_name, sizeof(files[count].iso_name));
...
static int mkiso_collect_files(const char *source_dir, mkiso_input_file_t **files_out, size_t *count_out,
    ...
    if(handle == INVALID_HANDLE_VALUE) {
        mkiso_set_path_error(error_buf, error_buf_size, "failed to open ", source_dir);
        return -1;
    }
    ...
    do {
        ...
        compat_path_join(full_path, sizeof(full_path), source_dir, find_data.cFileName);
        if(mkiso_add_file(full_path, find_data.cFileName, &files, &count, &capacity, error_buf,
                          error_buf_size) < 0) {
            FindClose(handle);
    ...
    while((entry = readdir(dir)) != NULL) {
        ...
        if(mkiso_add_file(full_path, entry->d_name, &files, &count, &capacity, error_buf, error_buf_size) <
           0) {
            closedir(dir);
            mkiso_free_files(files, count);
            return -1;
    ...
    if(!count) {
        free(files);
        mkiso_set_path_error(error_buf, error_buf_size, "no regular files found in ", source_dir);
        return -1;
    }
...
int main(int argc, char **argv) {
    ...
    if(system_area_path && mkiso_load_file(system_area_path, &system_area, &system_area_size) < 0) {
        fprintf(stderr, "mkiso-lite: failed to read %s\n", system_area_path);
        goto cleanup;
    }

    if(mkiso_collect_files(source_dir, &files, &file_count, error_buf, sizeof(error_buf)) < 0) {
        fprintf(stderr, "mkiso-lite: %s\n", error_buf);
        goto cleanup;
    }

    if(boot_path)
        iso_lite_normalize_file_name(mkiso_path_basename(boot_path), boot_name, sizeof(boot_name));
    ...
    if(iso_lite_write_image(output_path, &config, iso_files, file_count, &stats, error_buf,
                            sizeof(error_buf)) < 0) {
        fprintf(stderr, "mkiso-lite: %s\n", error_buf);
        goto cleanup;
    }
