/* make-cd/tools/wadcrypto/aes.c */
/* Minimal AES-128 (ECB block ops + CBC mode), public-domain implementation. */

#include "aes.h"
#include <string.h>

static const uint8_t sbox[256] = {
    };

static const uint8_t rsbox[256] = {
    };

static const uint8_t Rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

/* GF(2^8) helpers */
static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ ((x >> 7) * 0x1b));
}
static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for(int i = 0; i < 8; i++) {
        if(b & 1)
            p ^= a;
        uint8_t hi = a & 0x80;
        a <<= 1;
        if(hi)
            a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void aes128_key_expand(const uint8_t key[16], uint8_t rk[176]) {
    memcpy(rk, key, 16);
    uint8_t t[4];
    for(int i = 16; i < 176; i += 4) {
        t[0] = rk[i - 4];
        t[1] = rk[i - 3];
        t[2] = rk[i - 2];
        t[3] = rk[i - 1];
        if(i % 16 == 0) {
            uint8_t tmp = t[0];
            t[0] = t[1];
            t[1] = t[2];
            t[2] = t[3];
            t[3] = tmp; /* RotWord */
            t[0] = sbox[t[0]];
            t[1] = sbox[t[1]];
            t[2] = sbox[t[2]];
            t[3] = sbox[t[3]]; /* SubWord */
            t[0] ^= Rcon[i / 16];
        }
        rk[i + 0] = rk[i - 16 + 0] ^ t[0];
        rk[i + 1] = rk[i - 16 + 1] ^ t[1];
        rk[i + 2] = rk[i - 16 + 2] ^ t[2];
        rk[i + 3] = rk[i - 16 + 3] ^ t[3];
    }
}

/* State s is column-major: s[r + 4*c]; columns are contiguous (s[4c..4c+3]). */
static void add_round_key(uint8_t s[16], const uint8_t *rk) {
    for(int i = 0; i < 16; i++)
        s[i] ^= rk[i];
}
static void sub_bytes(uint8_t s[16]) {
    for(int i = 0; i < 16; i++)
        s[i] = sbox[s[i]];
}
static void inv_sub_bytes(uint8_t s[16]) {
    for(int i = 0; i < 16; i++)
        s[i] = rsbox[s[i]];
}

static void shift_rows(uint8_t s[16]) {
    uint8_t t;
    /* row 1: rotate left 1 */
    t = s[1];
    s[1] = s[5];
    s[5] = s[9];
    s[9] = s[13];
    s[13] = t;
    /* row 2: rotate left 2 */
    t = s[2];
    s[2] = s[10];
    s[10] = t;
    t = s[6];
    s[6] = s[14];
    s[14] = t;
    /* row 3: rotate left 3 (== right 1) */
    t = s[15];
    s[15] = s[11];
    s[11] = s[7];
    s[7] = s[3];
    s[3] = t;
}
static void inv_shift_rows(uint8_t s[16]) {
    uint8_t t;
    /* row 1: rotate right 1 */
    t = s[13];
    s[13] = s[9];
    s[9] = s[5];
    s[5] = s[1];
    s[1] = t;
    /* row 2: rotate right 2 */
    t = s[2];
    s[2] = s[10];
    s[10] = t;
    t = s[6];
    s[6] = s[14];
    s[14] = t;
    /* row 3: rotate right 3 (== left 1) */
    t = s[3];
    s[3] = s[7];
    s[7] = s[11];
    s[11] = s[15];
    s[15] = t;
}

static void mix_columns(uint8_t s[16]) {
    for(int c = 0; c < 4; c++) {
        uint8_t *a = s + 4 * c;
        uint8_t  a0 = a[0], a1 = a[1], a2 = a[2], a3 = a[3];
        a[0] = (uint8_t)(xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3);
        a[1] = (uint8_t)(a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3);
        a[2] = (uint8_t)(a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3));
        a[3] = (uint8_t)((xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3));
    }
}
static void inv_mix_columns(uint8_t s[16]) {
    for(int c = 0; c < 4; c++) {
        uint8_t *a = s + 4 * c;
        uint8_t  a0 = a[0], a1 = a[1], a2 = a[2], a3 = a[3];
        a[0] = (uint8_t)(gmul(a0, 14) ^ gmul(a1, 11) ^ gmul(a2, 13) ^ gmul(a3, 9));
        a[1] = (uint8_t)(gmul(a0, 9) ^ gmul(a1, 14) ^ gmul(a2, 11) ^ gmul(a3, 13));
        a[2] = (uint8_t)(gmul(a0, 13) ^ gmul(a1, 9) ^ gmul(a2, 14) ^ gmul(a3, 11));
        a[3] = (uint8_t)(gmul(a0, 11) ^ gmul(a1, 13) ^ gmul(a2, 9) ^ gmul(a3, 14));
    }
}

void aes128_encrypt_block(const uint8_t rk[176], uint8_t s[16]) {
    add_round_key(s, rk);
    for(int r = 1; r <= 9; r++) {
        sub_bytes(s);
        shift_rows(s);
        mix_columns(s);
        add_round_key(s, rk + 16 * r);
    }
    sub_bytes(s);
    shift_rows(s);
    add_round_key(s, rk + 160);
}

void aes128_decrypt_block(const uint8_t rk[176], uint8_t s[16]) {
    add_round_key(s, rk + 160);
    for(int r = 9; r >= 1; r--) {
        inv_shift_rows(s);
        inv_sub_bytes(s);
        add_round_key(s, rk + 16 * r);
        inv_mix_columns(s);
    }
    inv_shift_rows(s);
    inv_sub_bytes(s);
    add_round_key(s, rk);
}

void aes128_cbc_encrypt(const uint8_t key[16], const uint8_t iv[16], uint8_t *buf, size_t len) {
    uint8_t rk[176];
    aes128_key_expand(key, rk);
    uint8_t prev[16];
    memcpy(prev, iv, 16);
    for(size_t off = 0; off + 16 <= len; off += 16) {
        uint8_t *blk = buf + off;
        for(int i = 0; i < 16; i++)
            blk[i] ^= prev[i];
        aes128_encrypt_block(rk, blk);
        memcpy(prev, blk, 16);
    }
}

void aes128_cbc_decrypt(const uint8_t key[16], const uint8_t iv[16], uint8_t *buf, size_t len) {
    uint8_t rk[176];
    aes128_key_expand(key, rk);
    uint8_t prev[16];
    memcpy(prev, iv, 16);
    uint8_t cipher[16];
    for(size_t off = 0; off + 16 <= len; off += 16) {
        uint8_t *blk = buf + off;
        memcpy(cipher, blk, 16);
        aes128_decrypt_block(rk, blk);
        for(int i = 0; i < 16; i++)
            blk[i] ^= prev[i];
        memcpy(prev, cipher, 16);
    }
}
