...
static inline int tool_tm_fields_valid(const struct tm *tm_value) {
    ...
    return year >= 0 && year <= 9999 && tm_value->tm_mon >= 0 && tm_value->tm_mon <= 11 &&
           tm_value->tm_mday >= 1 && tm_value->tm_mday <= 31 && tm_value->tm_hour >= 0 &&
           tm_value->tm_hour <= 23 && tm_value->tm_min >= 0 && tm_value->tm_min <= 59 &&
...
static inline int tool_format_iso9660_volume_time(uint8_t out[17], const struct tm *tm_value) {
    unsigned year;
