...
KOSLOAD_IMPORT(sifcmd, 17, RpcRegister, void,
               (smap_iop_rpc_server_t * sd, int sid, smap_rpc_handler_t func, void *buf,
                smap_rpc_handler_t cfunc, void *cbuf, smap_iop_rpc_queue_t *qd));
KOSLOAD_IMPORT(sifcmd, 19, RpcSetQueue, smap_iop_rpc_queue_t *, (smap_iop_rpc_queue_t * qd, int thread_id));
KOSLOAD_IMPORT(sifcmd, 21, RpcExecRequest, void, (smap_iop_rpc_server_t * sd));
KOSLOAD_IMPORT_TABLE_END(sifcmd);

/* sysmem — shared-region allocation. */
KOSLOAD_IMPORT_TABLE(sysmem, 1, 1);
KOSLOAD_IMPORT(sysmem, 4, AllocSysMemory, void *, (int mode, int size, void *ptr));
...
#define SMAP_DEV9_BASE          0xb0000000
#define SMAP_DEV9_ID            (SMAP_DEV9_BASE + 0x0002)  /* u16 */
#define SMAP_DEV9_STATUS        (SMAP_DEV9_BASE + 0x0004)  /* u16 */
#define SMAP_DEV9_PIO_OUT       (SMAP_DEV9_BASE + 0x0102)  /* u8 */
#define SMAP_DEV9_INTR_VEC      (SMAP_DEV9_BASE + 0x0128)  /* u16 */

#define SMAP_INTR_STAT          (SMAP_DEV9_BASE + 0x0028)  /* u16, R/W */
#define SMAP_INTR_ENABLE        (SMAP_DEV9_BASE + 0x002a)  /* u16 */
#define SMAP_BD_MODE            (SMAP_DEV9_BASE + 0x0102)  /* u8 */
#define SMAP_INTR_CLR           (SMAP_DEV9_BASE + 0x0128)  /* u16, W1C */
#define SMAP_INTR_BITMSK        0x007C
#define SMAP_INTR_ENABLE_MASK   0x0078
#define SMAP_INTR_RXEND         0x0020
#define SMAP_INTR_TXEND         0x0010
#define SMAP_INTR_RXDNV         0x0008
#define SMAP_INTR_TXDNV         0x0004

#define SMAP_EMAC3_RESET_REG    (SMAP_DEV9_BASE + 0x1000)  /* u8 */
#define SMAP_EMAC3_FIFO_RST_REG (SMAP_DEV9_BASE + 0x1030)  /* u8 */

#define SMAP_STACR              (SMAP_DEV9_BASE + 0x2000)  /* u32 */

#define SMAP_EMAC3_MR1          (SMAP_DEV9_BASE + 0x2004)  /* u32 */
#define SMAP_EMAC3_TXMODE1      (SMAP_DEV9_BASE + 0x200c)  /* u32 */
#define SMAP_EMAC3_RXMODE       (SMAP_DEV9_BASE + 0x2010)  /* u32 */
#define SMAP_EMAC3_INTR_STAT    (SMAP_DEV9_BASE + 0x2014)  /* u32, W1C */
#define SMAP_EMAC3_INTR_ENABLE  (SMAP_DEV9_BASE + 0x2018)  /* u32 */
#define SMAP_EMAC3_ADDR_HI      (SMAP_DEV9_BASE + 0x201c)  /* u32 */
#define SMAP_EMAC3_ADDR_LO      (SMAP_DEV9_BASE + 0x2020)  /* u32 */
#define SMAP_EMAC3_PAUSE_TIMER  (SMAP_DEV9_BASE + 0x202c)  /* u32 */
#define SMAP_EMAC3_GROUP_HASH1  (SMAP_DEV9_BASE + 0x2040)  /* u32 */
#define SMAP_EMAC3_GROUP_HASH2  (SMAP_DEV9_BASE + 0x2044)  /* u32 */
#define SMAP_EMAC3_GROUP_HASH3  (SMAP_DEV9_BASE + 0x2048)  /* u32 */
#define SMAP_EMAC3_GROUP_HASH4  (SMAP_DEV9_BASE + 0x204c)  /* u32 */
#define SMAP_EMAC3_IFG          (SMAP_DEV9_BASE + 0x2058)  /* u32 */
#define SMAP_EMAC3_TX_THRESHOLD (SMAP_DEV9_BASE + 0x2060)  /* u32 */
#define SMAP_EMAC3_RX_WATERMARK (SMAP_DEV9_BASE + 0x2064)  /* u32 */

#define SMAP_MR1_INIT_LE          0x00008164
#define SMAP_TXMODE1_INIT_LE      0x0000380f
#define SMAP_RXMODE_INIT_LE       0x0000c058
#define SMAP_INTR_TX_ERR_BITS_LE  0x01c00000
#define SMAP_E3_INTR_ALL_LE       0x03fb03ff
#define SMAP_E3_DEAD_ALL_LE       0x03200000
#define SMAP_PAUSE_TIMER_INIT_LE  0xffff0000
#define SMAP_IFG_INIT_LE          0x00040000
#define SMAP_TX_THRESHOLD_INIT_LE 0x00006000
#define SMAP_RX_WATERMARK_INIT_LE 0x40000800

#define SMAP_EMAC3_MODE0          (SMAP_DEV9_BASE + 0x2000)  /* u32 */
#define SMAP_EMAC3_STA_CTRL       (SMAP_DEV9_BASE + 0x205c)  /* u32 */

#define SMAP_MODE0_ENABLE_LE      0x00001800

#define SMAP_PHY_CMD_READ         0x1000
#define SMAP_PHY_CMD_WRITE        0x2000
#define SMAP_PHY_ADDR_DSPHYTER    (1 << 5)

#define SMAP_PHY_BMCR             0x00
#define SMAP_PHY_BMSR             0x01
#define SMAP_PHY_ANAR             0x04
#define SMAP_PHY_ANLPAR           0x05
#define SMAP_PHY_PHYSTS           0x10

#define SMAP_BMCR_RST             (1 << 15)
#define SMAP_BMCR_100M            (1 << 13)
#define SMAP_BMCR_ANEN            (1 << 12)
#define SMAP_BMCR_DUPM            (1 << 8)

#define SMAP_BMSR_ANCP            (1 << 5)
#define SMAP_BMSR_LINK            (1 << 2)

#define SMAP_AN_10HDX             (1 << 5)
#define SMAP_AN_10FDX             (1 << 6)
#define SMAP_AN_100HDX            (1 << 7)
#define SMAP_AN_100FDX            (1 << 8)

#define SMAP_PHYSTS_ANCP          (1 << 4)
#define SMAP_PHYSTS_FDX           (1 << 2)
#define SMAP_PHYSTS_10M           (1 << 1)
#define SMAP_PHYSTS_LINK          (1 << 0)

#define SMAP_MODE1_FDX            (1u << 31)
#define SMAP_MODE1_FLOWCTRL       (1 << 28)
#define SMAP_MODE1_ALLOW_PF       (1 << 27)
#define SMAP_MODE1_IGNORE_SQE     (1 << 24)
#define SMAP_MODE1_MEDIA_MSK      (3 << 22)
#define SMAP_MODE1_MEDIA_10M      (0 << 22)
#define SMAP_MODE1_MEDIA_100M     (1 << 22)
...
#define SMAP_TXFIFO_CTRL        (SMAP_DEV9_BASE + 0x1000)
#define SMAP_TXFIFO_WR_PTR      (SMAP_DEV9_BASE + 0x1004)
#define SMAP_TXFIFO_FRAME_CNT   (SMAP_DEV9_BASE + 0x100C)
#define SMAP_TXFIFO_FRAME_INC   (SMAP_DEV9_BASE + 0x1010)
#define SMAP_TXFIFO_DATA        (SMAP_DEV9_BASE + 0x1100)

#define SMAP_RXFIFO_CTRL        (SMAP_DEV9_BASE + 0x1030)
#define SMAP_RXFIFO_RD_PTR      (SMAP_DEV9_BASE + 0x1034)
#define SMAP_RXFIFO_FRAME_CNT   (SMAP_DEV9_BASE + 0x103C)
#define SMAP_RXFIFO_FRAME_DEC   (SMAP_DEV9_BASE + 0x1040)
#define SMAP_RXFIFO_DATA        (SMAP_DEV9_BASE + 0x1200)

#define SMAP_EMAC3_TXMODE0      (SMAP_DEV9_BASE + 0x2008)
#define SMAP_TXMODE0_GNP_0_LE   0x00008000

#define SMAP_TXBUFBASE          0x1000
#define SMAP_TXBUFSIZE          0x1000
#define SMAP_RXBUFBASE          0x4000

#define SMAP_BD_MAX_ENTRY       64
#define SMAP_BD_TX_BASE         (SMAP_DEV9_BASE + 0x3000)
#define SMAP_BD_RX_BASE         (SMAP_DEV9_BASE + 0x3200)
#define SMAP_BD_STRIDE          8

#define SMAP_BD_TX_READY        0x8000
#define SMAP_BD_TX_GENFCS       0x0200
#define SMAP_BD_TX_GENPAD       0x0100
#define SMAP_BD_TX_ERRMASK      0x03FF
#define SMAP_BD_RX_EMPTY_BIT    0x8000
#define SMAP_BD_RX_ERRMASK      0x7FFF

#define SMAP_TXMAXSIZE          1514
#define SMAP_RXMINSIZE          14
#define SMAP_RXMAXSIZE          1518

#define SMAP_BD_BASE            (SMAP_DEV9_BASE + 0x3000)
#define SMAP_BD_TX_END          (SMAP_BD_BASE   + 0x0200)
#define SMAP_BD_RX_END          (SMAP_BD_BASE   + 0x0400)
#define SMAP_BD_RX_EMPTY        0x8000

#define SMAP_IRQ_NUM            0x7c

#define SMAP_RESET_RETRIES      1000
#define SMAP_STACR_RETRIES      1000

#define SMAP_STACR_INIT         0x00002000
#define SMAP_STACR_READY        0x20000000
...
#define SMAP_LINK_POLL_TICKS    500
...
static unsigned char g_rpc_buf[PS2_SMAP_RPC_BUF_SIZE] __attribute__((aligned(64)));
 ...
 * `g_smap_mac` writes. */
static __attribute__((noinline, noipa)) void smap_iop_publish_word_barrier(volatile unsigned int *probe) {
    __asm__ volatile("" ::: "memory");
    (void)*probe;
    __asm__ volatile("" ::: "memory");
}

static __attribute__((noinline, noipa)) void smap_publish_mac_bytes(volatile unsigned char *dst,
                                                                    const unsigned char    *src) {
    unsigned int hi = (unsigned int)src[0] | ((unsigned int)src[1] << 8) | ((unsigned int)src[2] << 16) |
                      ((unsigned int)src[3] << 24);
    unsigned short lo = (unsigned short)src[4] | (unsigned short)((unsigned int)src[5] << 8);
...
static int smap_emac3_soft_reset(void) {
    ...
    for(i = 0; i < SMAP_RESET_RETRIES; i++) {
        if((REG8(SMAP_EMAC3_RESET_REG) & 1) == 0)
            return 0;
    }
...
static int smap_fifo_reset(void) {
    ...
    for(i = 0; i < SMAP_RESET_RETRIES; i++) {
        if((REG8(SMAP_EMAC3_FIFO_RST_REG) & 1) == 0)
            return 0;
    }
...
static void smap_interrupts_disable_clear(void) {
    ...
    REG32(SMAP_EMAC3_INTR_ENABLE) = 0;
...
static void smap_descriptors_zero(void) {
    ...
    for(addr = SMAP_BD_BASE; addr < SMAP_BD_TX_END; addr += 8) {
        REG16(addr + 0) = 0;
        REG16(addr + 2) = 0;
        REG16(addr + 4) = 0;
        REG16(addr + 6) = 0;
    ...
    for(addr = SMAP_BD_TX_END; addr < SMAP_BD_RX_END; addr += 8) {
...
static int smap_get_node_addr(struct smap_chan_iop *smap) {
    ...
    smap->hwaddr[0] = (unsigned char)(buf[0] & 0xff);
    smap->hwaddr[1] = (unsigned char)((buf[0] >> 8) & 0xff);
    smap->hwaddr[2] = (unsigned char)(buf[1] & 0xff);
    smap->hwaddr[3] = (unsigned char)((buf[1] >> 8) & 0xff);
    smap->hwaddr[4] = (unsigned char)(buf[2] & 0xff);
    smap->hwaddr[5] = (unsigned char)((buf[2] >> 8) & 0xff);
...
static void smap_emac3_program_mode(const struct smap_chan_iop *smap) {
    ...
    mac01 = smap_byteswap_halfword(((unsigned int)smap->hwaddr[1] << 8) | (unsigned int)smap->hwaddr[0]);
    mac23 = smap_byteswap_halfword(((unsigned int)smap->hwaddr[3] << 8) | (unsigned int)smap->hwaddr[2]);
    mac45 = smap_byteswap_halfword(((unsigned int)smap->hwaddr[5] << 8) | (unsigned int)smap->hwaddr[4]);

    addr_hi = mac01 << 16;
    REG32(SMAP_EMAC3_ADDR_HI) = addr_hi;
    (void)REG32(SMAP_EMAC3_ADDR_HI);

    addr_lo = (mac45 << 16) | (mac23 & 0xffff);
    ...
    REG32(SMAP_EMAC3_GROUP_HASH1) = 0;
    (void)REG32(SMAP_EMAC3_GROUP_HASH1);
    REG32(SMAP_EMAC3_GROUP_HASH2) = 0;
    (void)REG32(SMAP_EMAC3_GROUP_HASH2);
    REG32(SMAP_EMAC3_GROUP_HASH3) = 0;
    (void)REG32(SMAP_EMAC3_GROUP_HASH3);
    REG32(SMAP_EMAC3_GROUP_HASH4) = 0;
...
static int smap_phy_write(unsigned int reg, unsigned int data) {
    ...
    cmd_le = (data << 16) | SMAP_PHY_CMD_WRITE | SMAP_PHY_ADDR_DSPHYTER | (reg & 0x1f);
...
static int smap_phy_read(unsigned int reg, unsigned int *data_out) {
    ...
    cmd_le = SMAP_PHY_CMD_READ | SMAP_PHY_ADDR_DSPHYTER | (reg & 0x1f);
...
static int smap_phy_reset(void) {
    ...
    if(smap_phy_write(SMAP_PHY_BMCR, SMAP_BMCR_RST | SMAP_BMCR_100M | SMAP_BMCR_ANEN | SMAP_BMCR_DUPM) < 0)
        return -1;
...
static void smap_apply_link_mode(unsigned int link_state) {
    ...
    if(link_state == PS2_SMAP_LINK_100FDX || link_state == PS2_SMAP_LINK_10FDX) {
    ...
    if(link_state == PS2_SMAP_LINK_10HDX || link_state == PS2_SMAP_LINK_10FDX)
...
static unsigned int smap_phy_poll_link(void) {
    ...
    if(smap_phy_read(SMAP_PHY_PHYSTS, &physts) == 0 && (physts & SMAP_PHYSTS_LINK) != 0) {
    ...
    while(smap->txbdusedcnt > 0) {
        ...
        if(g_hot_diag) {
            if((ctrl_stat & SMAP_BD_TX_ERRMASK) != 0)
                ...
                g_hot_diag->tx_packets = g_hot_diag->tx_packets + 1;
        }
...
static int smap_iop_send(struct smap_chan_iop *smap, const unsigned char *frame, unsigned int total_len) {
    unsigned int txlen;
    unsigned int bd_addr;
    unsigned int i;
    ...
    {
        const volatile unsigned int *src_w = (const volatile unsigned int *)(const volatile void *)frame;
        unsigned int full_words = total_len & ~3;
        unsigned int tail_bytes = total_len & 3;

        REG16(SMAP_TXFIFO_WR_PTR) = (unsigned short)(smap->txbwp & 0x0FFC);
        for(i = 0; i < full_words; i += 4)
            REG32(SMAP_TXFIFO_DATA) = *src_w++;
        if(tail_bytes != 0) {
            const volatile unsigned char *tail_b = (const volatile unsigned char *)src_w;
            unsigned int last = 0;
            if(tail_bytes >= 1)
                last |= (unsigned int)tail_b[0];
            if(tail_bytes >= 2)
                last |= (unsigned int)tail_b[1] << 8;
            if(tail_bytes >= 3)
                last |= (unsigned int)tail_b[2] << 16;
    ...
    REG8(SMAP_TXFIFO_FRAME_INC) = 1;
    smap_bd_set_ctrl_stat(bd_addr, SMAP_BD_TX_READY | SMAP_BD_TX_GENFCS | SMAP_BD_TX_GENPAD);
    ...
    smap->txbwp = SMAP_TXBUFBASE + ((smap->txbwp + txlen) % SMAP_TXBUFSIZE);
...
static unsigned int smap_iop_drain_rx(struct smap_chan_iop *smap) {
    ...
    while(1) {
        ...
        if((rxstat & SMAP_BD_RX_ERRMASK) != 0) {
            g_hot_diag->rx_drops_fcs = g_hot_diag->rx_drops_fcs + 1;
            goto advance;
        }
        ...
        next_head = (g_hot_diag->rx_head + 1) % PS2_SMAP_RX_SLOTS;
        if(next_head == (g_hot_diag->rx_tail_iop_view % PS2_SMAP_RX_SLOTS) &&
           g_hot_diag->rx_head != g_hot_diag->rx_tail_iop_view) {
            /* Drain into nowhere. */
            REG16(SMAP_RXFIFO_RD_PTR) = (unsigned short)(smap->rxbrp & 0x3FFC);
            for(i = 0; i < rxlen; i += 4)
                (void)REG32(SMAP_RXFIFO_DATA);
            g_hot_diag->rx_drops_no_slot = g_hot_diag->rx_drops_no_slot + 1;
            goto advance;
        }
...

        {
            volatile unsigned int *frame_dst_w =
                (volatile unsigned int *)(volatile void *)(g_rx_data + slot * g_rx_slot_size);

            REG16(SMAP_RXFIFO_RD_PTR) = (unsigned short)(smap->rxbrp & 0x3FFC);
            for(i = 0; i < rxlen; i += 4)
                *frame_dst_w++ = REG32(SMAP_RXFIFO_DATA);

            smap_iop_publish_word_barrier((volatile unsigned int *)(g_rx_data + slot * g_rx_slot_size));
            g_rx_ring[slot].len = pkt_len;
            g_rx_ring[slot].status = 0;
            g_rx_seq[slot] = g_rx_seq[slot] + 1;
            g_rx_ring[slot].seq = g_rx_seq[slot];
            smap_iop_publish_word_barrier(&g_rx_ring[slot].seq);

            g_hot_diag->rx_head = g_hot_diag->rx_head + 1;
            smap_iop_publish_word_barrier(&g_hot_diag->rx_head);
            g_hot_diag->rx_packets = g_hot_diag->rx_packets + 1;
        }
        delivered++;

    advance:
        REG8(SMAP_RXFIFO_FRAME_DEC) = 1;
        smap_bd_set_length(bd_addr, 0);
        smap_bd_set_pointer(bd_addr, 0);
...
static int smap_cdvd_read_rtc(unsigned int *timestamp) {
    ...
    jst_unix = smap_datetime_to_unix(2000 + (unsigned int)year, (unsigned int)month, (unsigned int)day,
...
static int smap_cdvd_write_rtc(unsigned int timestamp) {
    ...
    smap_unix_to_datetime(jst_unix, &year, &month, &day, &hour, &minute, &second);
    ...
    clock.year = smap_int_to_bcd(year - 2000);
...
static void *smap_rpc_handler(int fno, void *buffer, int length) {
    ...
    case PS2_SMAP_FNO_GET_DIAG: {
        ...
        g_cold_diag_rsp.ring_invariant_flags = 0;
        for(i = 0; i < 6; i++)
    ...
    case PS2_SMAP_FNO_RELEASE_RX: {
        ps2_smap_release_rx_req_t *req = (ps2_smap_release_rx_req_t *)buffer;
        ...
        if(req->slot >= PS2_SMAP_RX_SLOTS) {
             ...
             * overflow handling. */
            g_hot_diag->rx_tail_iop_view = g_hot_diag->rx_tail_iop_view + 1;
            g_hot_diag->last_release_batch_size = 1;
    ...
    case PS2_SMAP_FNO_RELEASE_RX_BATCH: {
        ps2_smap_release_rx_batch_req_t *req = (ps2_smap_release_rx_batch_req_t *)buffer;
        unsigned int i;
        ...
        if(req->count == 0 || req->count > PS2_SMAP_RELEASE_BATCH_MAX) {
            g_last_validation_op = (unsigned int)fno;
            g_last_validation_slot = 0;
            ...
            if(rc_local == (int)PS2_SMAP_RC_OK) {
                g_hot_diag->rx_tail_iop_view = g_hot_diag->rx_tail_iop_view + req->count;
    ...
    case PS2_SMAP_FNO_SUBMIT_TX: {
        ps2_smap_submit_tx_req_t *req = (ps2_smap_submit_tx_req_t *)buffer;
        zero_bytes(&g_rc_rsp, sizeof(g_rc_rsp));
        if(req->slot >= PS2_SMAP_TX_SLOTS) {
            ...
            g_rc_rsp.rc = (int)PS2_SMAP_RC_BAD_SLOT;
        } else if(req->len < (unsigned int)SMAP_RXMINSIZE || req->len > PS2_SMAP_FRAME_MAX) {
            g_last_validation_op = (unsigned int)fno;
            g_last_validation_slot = req->slot;
            g_last_validation_seq = req->seq;
            g_last_validation_len = req->len;
            ...
            g_last_validation_len = req->len;
            g_rc_rsp.rc = (int)PS2_SMAP_RC_BAD_SEQ;
        } else if(g_hot_diag->tx_slot_state[req->slot] != PS2_SMAP_TX_FREE) {
            g_last_validation_op = (unsigned int)fno;
            g_last_validation_slot = req->slot;
            g_last_validation_seq = req->seq;
            g_last_validation_len = req->len;
            ...
            g_tx_seq[req->slot] = g_tx_seq[req->slot] + 1;
            g_hot_diag->tx_slot_state[req->slot] = PS2_SMAP_TX_BUSY;
            g_hot_diag->last_submit_batch_size = 1;
            g_rc_rsp.rc = (int)PS2_SMAP_RC_OK;
        }
        out_ptr = (void *)&g_rc_rsp;
        result_rc = g_rc_rsp.rc;
    ...
    case PS2_SMAP_FNO_SUBMIT_TX_BATCH: {
        ps2_smap_submit_tx_batch_req_t *req = (ps2_smap_submit_tx_batch_req_t *)buffer;
        unsigned int i;
        int rc_local = (int)PS2_SMAP_RC_OK;
        zero_bytes(&g_rc_rsp, sizeof(g_rc_rsp));
        ...
        if(req->count == 0 || req->count > PS2_SMAP_SUBMIT_BATCH_MAX) {
            g_last_validation_op = (unsigned int)fno;
            g_last_validation_slot = 0;
            g_last_validation_seq = req->count;
            g_last_validation_len = 0;
            ...
            for(i = 0; i < req->count; i++) {
                ...
                if(len < (unsigned int)SMAP_RXMINSIZE || len > PS2_SMAP_FRAME_MAX) {
                    g_last_validation_op = (unsigned int)fno;
                    g_last_validation_slot = slot;
                    g_last_validation_seq = seq;
...
static void smap_register_queue_workaround(smap_iop_rpc_queue_t *qd) {
    static const kosload_iop_link_table_header_t sifcmd_query
        __attribute__((aligned(4))) = {0, 0, 0x0101, 0, {'s', 'i', 'f', 'c', 'm', 'd', 0, 0}};
    void **funcs = QueryLibraryEntryTable(&sifcmd_query);
    if(funcs == 0)
        return;

    unsigned int sifcmd_base = (unsigned int)funcs - 0x1574;
    volatile unsigned int *aq_ptr = (volatile unsigned int *)(sifcmd_base + 0x2a60);
...
static void smap_rpc_thread(void *arg) {
    ...
    RpcRegister(&g_server, PS2_SMAP_RPC_ID, smap_rpc_handler, g_rpc_buf, 0, 0, &g_queue);
    ...
    *(volatile unsigned int *)(0xA0000000 | PS2_SMAP_READY_IOP_PHYS) = PS2_SMAP_READY_MAGIC;

    {
        volatile smap_iop_rpc_queue_t *vqd = (volatile smap_iop_rpc_queue_t *)&g_queue;
        for(;;) {
            if(vqd->start != 0) {
                smap_iop_rpc_server_t *pending = (smap_iop_rpc_server_t *)vqd->start;
...
static void smap_pump_thread(void *arg) {
    ...
    for(;;) {
        ...
        if(g_hot_diag != 0) {
            g_hot_diag->heartbeat = g_hot_diag->heartbeat + 1;
            g_hot_diag->link_state = g_smap_link_state;
        }

        if(link_poll_ticks == 0) {
...
static int smap_alloc_shared_region(void) {
    ...
    unsigned int diag_size = (sizeof(ps2_smap_hot_diag_t) + 15) & ~15;
    ...
    rx_ring_offset = 0;
    rx_data_offset = (rx_ring_offset + rx_ring_size + 15) & ~15;
    tx_data_offset = (rx_data_offset + rx_data_size + 15) & ~15;
    diag_offset = (tx_data_offset + tx_data_size + 15) & ~15;
    total_size = (diag_offset + diag_size + 15) & ~15;

    /* AllocSysMemory rounds size up to 256-byte boundary internally,
     * but we round up here too so total_size is a known multiple. */
    total_size = (total_size + 0xff) & ~0xff;
    ...
    g_shared_iop_phys = (unsigned int)base & 0x1fffff;
    g_shared_size = total_size;

    g_rx_ring = (volatile ps2_smap_rx_desc_t *)(g_shared_base + rx_ring_offset);
    g_rx_data = g_shared_base + rx_data_offset;
    g_tx_data = g_shared_base + tx_data_offset;
    g_hot_diag = (volatile ps2_smap_hot_diag_t *)(g_shared_base + diag_offset);
    ...
    g_layout.tx_state_offset =
        diag_offset +
        (unsigned int)(((unsigned char *)&((ps2_smap_hot_diag_t *)0)->tx_slot_state[0] - (unsigned char *)0));
...
int _start(int argc, char **argv) {
    ...
    if(argc >= 2 && argv != 0 && argv[1] != 0 && parse_mac_arg(argv[1], g_mac_override) == 0 &&
