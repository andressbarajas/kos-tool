 ...
 * program. */
static uint32_t g_last_link_state = 0xffffffff;
...
static void smap_diag_dump(void) {
    ...
    p = buf;
    p = append_str_d(p, "rx:");
    p = append_hex4(p, hot.rx_packets);
    p = append_str_d(p, " tx:");
    p = append_hex4(p, hot.tx_packets);
    p = append_str_d(p, " hd:");
    p = append_hex4(p, hot.rx_head);
    p = append_str_d(p, " tl:");
    p = append_hex4(p, hot.ee_rx_tail);
    p = append_str_d(p, " iv:");
    p = append_hex4(p, hot.rx_tail_iop_view);
    p = append_str_d(p, " hb:");
    ...
    *p = '\0';
    ...
    p = buf;
    p = append_str_d(p, "lk:");
    p = append_hex4(p, hot.link_state);
    p = append_str_d(p, " dr:");
    p = append_hex4(p, hot.rx_drops_no_slot);
    p = append_str_d(p, " fc:");
    p = append_hex4(p, hot.rx_drops_fcs);
    p = append_str_d(p, " un:");
    p = append_hex4(p, hot.tx_underruns);
    p = append_str_d(p, " op:");
    p = append_hex4(p, hot.last_rpc_op);
    p = append_str_d(p, " rc:");
    p = append_hex4(p, (uint32_t)hot.last_rpc_result);
    p = append_str_d(p, " rb:");
    p = append_hex4(p, hot.last_release_batch_size);
    p = append_str_d(p, " sb:");
    ...
    *p = '\0';
    ...
    p = append_str_d(p, "ts:");
    {
        ...
        for(i = 0; i < 16; i++) {
            ...
            *p++ = hex_d[hot.tx_slot_state[i] & 0xf];
        }
    }
    p = append_str_d(p, " ps:");
    p = append_hex4(p, hot.ee_pending_submits);
    p = append_str_d(p, " pr:");
...
static void smap_loop_adapter(bool is_main_loop) {
    ...
    if(is_main_loop) {
        ...
        if(!g_loader_settled) {
            ...
            for(;;) {
                if(ps2_smap_get_hot_snapshot(&hot) >= 0 && hot.link_state != PS2_SMAP_LINK_DOWN)
                    break;
            }
