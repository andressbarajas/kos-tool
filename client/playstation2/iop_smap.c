...
#define EE_IOP_UNCACHED(iop_phys) ((volatile void *)(0xBC000000 | ((uint32_t)(iop_phys) & 0x1FFFFF)))

/* Maximum number of bind retries.  ps2-load-ip's other clients use 64. */
#define SMAP_BIND_ATTEMPTS 64
...
static volatile ee_sif_rpc_client_t smap_client __attribute__((aligned(64)));
...
static int rpc_call_sync(int fno, const void *send, uint32_t send_size, void *recv, uint32_t recv_size) {
    return ee_sif_rpc_call(&smap_client, fno, send, send_size, recv, recv_size);
}
...
int ps2_smap_init(void) {
    ...
    {
        volatile uint32_t *ready = (volatile uint32_t *)EE_IOP_UNCACHED(PS2_SMAP_READY_IOP_PHYS);
        uint32_t spins = 0;
    ...
    if(ee_sif_rpc_bind_retry(&smap_client, PS2_SMAP_RPC_ID, SMAP_BIND_ATTEMPTS) < 0)
        return -1;

    memset(&smap_layout_rsp, 0, sizeof(smap_layout_rsp));
    rc = rpc_call_sync(PS2_SMAP_FNO_GET_LAYOUT, 0, 0, &smap_layout_rsp, sizeof(smap_layout_rsp));
    if(rc < 0)
        return -2;
    ...
    g_hot_diag_view = (volatile ps2_smap_hot_diag_t *)EE_IOP_UNCACHED(g_shared_iop_phys + g_diag_offset);
    g_rx_ring_view = (volatile ps2_smap_rx_desc_t *)EE_IOP_UNCACHED(g_shared_iop_phys + g_rx_ring_offset);
    g_rx_data_view = (volatile uint8_t *)EE_IOP_UNCACHED(g_shared_iop_phys + g_rx_data_offset);
    g_tx_state_view = (volatile uint32_t *)EE_IOP_UNCACHED(g_shared_iop_phys + g_tx_state_offset);

    g_rx_tail = 0;
    g_tx_next_slot = 0;
    g_pending_release_count = 0;
    g_pending_submit_count = 0;
    g_tx_claimed_mask = 0;
    for(i = 0; i < 16; i++)
        g_ee_tx_seq[i] = 0;
    ...
    rc = rpc_call_sync(PS2_SMAP_FNO_GET_STATUS, 0, 0, &smap_status_rsp, sizeof(smap_status_rsp));
...
int ps2_smap_get_diag(ps2_smap_cold_diag_rsp_t *out) {
    ...
    rc = rpc_call_sync(PS2_SMAP_FNO_GET_DIAG, 0, 0, &smap_cold_diag_rsp, sizeof(smap_cold_diag_rsp));
    if(rc < 0)
        return -2;
...
int ps2_smap_get_rtc(uint32_t *unix_timestamp) {
    ...
    rc = rpc_call_sync(PS2_SMAP_FNO_GET_RTC, 0, 0, &smap_rtc_rsp, sizeof(smap_rtc_rsp));
    if(rc < 0)
        return -2;
...
int ps2_smap_set_rtc(uint32_t unix_timestamp) {
    ...
    rc = rpc_call_sync(PS2_SMAP_FNO_SET_RTC, &smap_rtc_req, sizeof(smap_rtc_req), &smap_rc_rsp,
                       sizeof(smap_rc_rsp));
    if(rc < 0)
        return -2;
...
static int ps2_smap_submit_tx_single(uint32_t slot, uint32_t len, uint32_t seq) {
    ...
    memset(&smap_rc_rsp, 0, sizeof(smap_rc_rsp));
    rc = rpc_call_sync(PS2_SMAP_FNO_SUBMIT_TX, &smap_submit_tx_req, sizeof(smap_submit_tx_req), &smap_rc_rsp,
                       sizeof(smap_rc_rsp));
    if(rc < 0)
        return -5;
    if(smap_rc_rsp.rc != PS2_SMAP_RC_OK)
...
static int ps2_smap_flush_tx(void) {
    ...
    memset(&smap_rc_rsp, 0, sizeof(smap_rc_rsp));
    rc = rpc_call_sync(PS2_SMAP_FNO_SUBMIT_TX_BATCH, &smap_submit_batch_req, sizeof(smap_submit_batch_req),
     ...
     * expected ones. */
    for(i = 0; i < g_pending_submit_count; i++) {
        int rc_one = ps2_smap_submit_tx_single(g_pending_submits[i].slot, g_pending_submits[i].len,
...
int ps2_smap_send(const void *frame, uint32_t len) {
    ...
    if(!g_layout_valid || frame == 0)
        return -1;
    if(len < 14 || len > PS2_SMAP_FRAME_MAX)
        return -2;
    ...
    tx_data_iop = g_shared_iop_phys + g_tx_data_offset + slot * g_tx_slot_size;
     ...
     * while it's queued. */
    expected_seq = (slot < 16) ? g_ee_tx_seq[slot] : 0;
    ...
    if(slot < 16)
        g_ee_tx_seq[slot] = expected_seq + 1;
...
int ps2_smap_release_pending(void) {
    ...
    if(g_pending_release_count != 0) {
        ...
        memset(&smap_rc_rsp, 0, sizeof(smap_rc_rsp));
        rc = rpc_call_sync(PS2_SMAP_FNO_RELEASE_RX_BATCH, &smap_release_batch_req,
                           sizeof(smap_release_batch_req), &smap_rc_rsp, sizeof(smap_rc_rsp));
        g_pending_release_count = 0;
        if(rc < 0)
            rc_release = -2;
