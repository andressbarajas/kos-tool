...
#define GAPS_RX_IO_AREA (0x80000000 | RTL_MEM)
#define GAPS_TX_IO_AREA (0x80000000 | RTL_MEM)
...
int rtl_bb_detect(void) {
    // This pointer's data is always aligned to 4 bytes--just look at the
    // register address!
...
static int rtl_init(void) {
     ...
     */
    nic8[RT_CONFIG1] =
        (nic8[RT_CONFIG1] & ~(RT_CONFIG1_LWACT | RT_CONFIG1_LED0)) | RT_CONFIG1_DVRLOAD | RT_CONFIG1_LED1;
...
static void rtl_link_change(void) {
    ...
    if(nic16[RT_MII_BMSR / 2] & RT_MII_LINK) {
         ...
         * timeout immediately upon bringing link back up, so we can retry
         * immediately. */
        if(timeout_loop > 0) {
            dhcp_attempts = 0;
            timeout_loop = -1;
...
int rtl_bb_init(void) {
    ...
    // The BBA uses the range 0x01840000-0x0184ffff for TX and RX (with usage
    // above 0x01848000 apparently for GAPS DMA), plus the 2 bytes at 0x0183fffc
    // for... something
    ...
    /* Register offset 0x142c controls where the image area at GAPS offset
     * 0x8000 (so 0x01848000) points to, which is used by DMA */
    g232[0x1414 / 4] = 0x00000001;  // Interrupt-related...? (Whatever this is, it gets 1
                                    // written to it a couple times, and gets a zero written to
                                    // it in the same places where 0x1418 gets the turn-off
                                    // code...)
    g232[0x1434 / 4] = 0x00000001;
    // There appears to be a register at offset 0x1410, which has 0x0c003000 in
    // it. This appears to be a pretty random address in the syscall area. It
    // doesn't seem to be used for anything, though, and it just seems to get
    // read from in its unused function--it's never written to. Similarly,
    // 0x1430 has 0x00000010 in it, who knows what for. This one doesn't even
    // have an unused function for it.

    // Now configure the RTL8139's PCI configuration

    // VEN:DEV is 11db:1234 (vendor code is "Sega Enterprises, LTD")
    // The GAPS bridge is really just an MMU with a memory buffer that maps the
    // RTL8139C to the Dreamcast's memory space, so these are actually the PCI
    // configuration registers for the RTL8139, not GAPS (those are just the
    // 0x1400 regs). It has a custom ven:dev ID, but the class ID in 0x1608
    // indicates a network controller (byte 0x160b = 0x02 = network controller,
    // 0x160a = 0x00 = Ethernet controller) See PCI Local Bus Specification 2.2
    // (2.3 has all the 2.2 stuff in it and the RTL8139C uses 2.2) This is also
    // documented in the RTL8139C's datasheet, under "PCI Configuration Space
    // Registers"
    ...
    g216[0x1604 / 2] |= 0x6;        // PCI Command Register (note: Fast Back-to-Back
                                    // is read-only 0 on this bridge)
    // 0x1610 (BAR0, I/O BAR) reads back as 0x00000001, which is I/O Space
    // Indicator
    g232[0x1614 / 4] = 0x01000000;  // BAR1 (Memory BAR)
    // There are two extra regs here, though, that are GAPS-specific (0x1650 and
    // 0x1654).
    if(g28[0x1650] & 1)  // ???
    {
    ...
    CacheBlockPurge((void *)0x01840000,
                    32768 / 32);  // Write back and invalidate the cache over
                                  // that area since it's volatile

    // do this weird dance
    // Appears to set and check some magic numbers to know if it's being initted
    // properly...?
    if(g232[0x141c / 4] == 0x41474553)  // Hah, this is ASCII for 'SEGA' in little-endian
    {
        ...
        if(g232[0x141c / 4] == 0x55aaff00) {
            ...
            if(g232[0x141c / 4] == 0xaa5500ff) {
                ...
                // causes the EEPROM to autoload all the registers initially. So
...
int rtl_bb_tx(unsigned char *pkt,
              int            len)  // pg. 15 in RTL8139C datasheet:
                        // http://realtek.info/pdf/rtl8139cp.pdf
{
    ...
    while(!(nic32[RT_TXSTATUS0 / 4 + rtl.cur_tx] &
            RT_TX_HOST_OWNS)) {                                     // While tx is not complete (checking OWN)
        if(nic32[RT_TXSTATUS0 / 4 + rtl.cur_tx] & RT_TX_ABORTED) {  // Check for abort
            // Found another bug: (nic32[RT_TXSTATUS0/4 + rtl.cur_tx] |= 1; //
            // <-- If abort, set descriptor size to 1)
            // |= the length to 1 doesn't do anything if the length is an odd
            // number >= 1... Should probably be RT_TXCONFIG register |= 1,
            // which clears abort state and retransmits, see pg 21 of RTL8139C
            // or pg 17 of RTL8139D datasheet
...
// Tx time
    ...
    if(len < 60)  // This condition may look a little gnarly, but that's because
                  // it's meant for speed above all else.
    {
        // So pad it.
        //
        // We absolutely need to pad this, otherwise prior packets can leak into
        // the padding. It's called "EtherLeak," and the RTL8139 is apparently a
        // poster child chipset for the issues that come from lacking auto-pad.
        ...
        unsigned int copyback_pkt_extras_end =
            (copyback_pkt_offset_len + 3) & -4;  // This is either equal to or 4 bytes greater than
                                                 // copyback_pkt_len_align4
        ...
        if(copyback_pkt_extras) {
            ...
            // One 4-byte write instead of up to 3x 1-byte writes. For uncached
            // setups, this makes odd-size small packets take the same total
            // time to copy to the NIC as multiple-of-4-sized packets, meaning
            // this is all running as fast as possible.
            *(unsigned int *)(copyback_pkt_base + output_offset) = last_data;
        }

        // Pad the rest of the packet with zeros, if more trailing zeroes need
        // to be written beyond the mixed bytes
        if(copyback_pkt_extras_end < 64)  // The only time this won't be true for small packets is for one
                                          // with 17 payload bytes.
        {
            // 2 unmodified bytes will end up being written to the RTL's TX
            // buffer (since we just want 60 bytes, which offsets to 62, and 62
            // then 4-byte-aligns to 64. Offset everything back 2 bytes to undo
            // the internal ethernet alignment for transmit results in bytes
            // 65-66 getting copied unzeroed), but that's OK here. The RTL8139C
            // will clobber them with the second half of a 4-byte CRC because
            // it's been configured to append a CRC, anyways--not to mention we
            // set a length parameter for transmit, which makes those bytes
            // doubly irrelevant.
            ...
            __asm__ volatile("clrs\n"          // Align for parallelism (CO) - SH4a use "stc SR, Rn"
                                               // instead with a dummy Rn
                             "dt %[size]\n\t"  // Decrement and test size here once to
                                               // prevent extra jump (EX 1)
                             ".align 2\n"
                             "1:\n\t"
                             // *--nextd = val
                             "mov.l %[data], @-%[out]\n\t"  // (LS 1/1)
            ...
            //
            // GCC does a mediocre job of optimizing memset on SH4 for some
        ...
        // Note that 60 bytes would be offset by 62, still within 2nd cache
        // block But we write to 64 bytes... which is still within the 2nd cache
        // block ;).
        CacheBlockWriteBack((unsigned char *)(copyback_pkt_base + 32), 1);

        len = 60;  // Finally, set length for transmit

        // NOTE: The reason the minimum length is hardcoded to 60 is because the
        // minimum frame size allowed is 46 bytes + 14 byte ethernet header.
        // Well, it's actually 64, but the NIC is configured to auto-append a
        // 4-byte CRC.
    }

    // Copy packet over to RTL via GAPS while also accounting for dcload-ip's
    // packet alignment offset
    SH4_mem_to_pkt_X_movca_32((unsigned char *)0x81848000, copyback_pkt_base, len);
    // Technically this will prefetch beyond 1536 for packets between 1504 and
    // 1514 in size, but that's not an issue.
...
// Tx time end
    ...
    // Set len (SIZE field), destructively zeroing out all other R/W settings.
    // OWN needs to be cleared by software; it does here. Software writes don't
    // impact the read-only bits. Zeroing also sets Early FIFO TX threshold to 8
    // bytes. Finally, writing to the status register triggers the packet send.
...
static void pktcpy(unsigned char *dest, unsigned char *src,
                   unsigned int n)  // dest and src should already be in a
                                    // copyback memory region
{
...
static int rtl_bb_rx() {
    ...
    while(!(nic8[RT_CHIPCMD] & RT_CMD_RX_BUF_EMPTY)) {
        ...
        // This also means we don't have to worry about invalidating a block
        // holding the status byte. Nice!
        ...
        if(rx_size == RT_RX_STATUS_EARLY) {
            rtl_is_copying = true;  // Really don't want to run a DHCP renewal
...
// Rx time
            ...
            pktcpy(raw_current_pkt, pkt,
                   pkt_size);  // SH4_pkt_to_mem() will shift it by 2 for
                               // current_pkt
...
// Process time end
        ...
        // Align next packet to 4-bytes (add 4 to account for transmit status;
        // the 4 extra bytes included in rx_size are the CRC)
        ...
        if(rtl.cur_rx >= RX_BUFFER_LEN) {
            ...
            nic16[RT_RXBUFTAIL / 2] = RTL_RX_CAPR_WRAP;
            // According to the RTL8139C datasheet, 0xfff0 = 65520 is the
            // default value of the register, and the register cannot be written
            // to before data has been read from the buffer for some reason. So,
            // presumably, we can just use that value here.
            //
            // Although, in the specific case of this system with the GAPS PCI
            // Bridge, we can also just use 0x7ff0 since that's a well-known
            // memory location (it's the last 16 bytes of the last txdesc.
            // Because each txdesc is 2048 bytes and no more than 1536 bytes
            // will ever be written there, it seems like a pretty safe place to
            // put... whatever that apparently 100% necessary offset of -16 is
            // for).
        } else {
            rtl.cur_rx %= RX_BUFFER_LEN;
            nic16[RT_RXBUFTAIL / 2] = rtl.cur_rx - RTL_RX_CAPR_OFFSET;
            // Why 16? NetBSD and Linux do this, too. Status is 4, CRC appended
            // is 4, what's the other 8? Things don't work if this isn't 16,
            // anyways (I tried changing it). Maybe this is 16 for DMA reasons?
            // RealTek does it here:
            // https://www.cs.usfca.edu/~cruse/cs326f04/RTL8139_ProgrammersGuide.pdf
            // Maybe this is why 16: initial value is 0x0fff0 according to the
            // RTL8139C datasheet:
...
void rtl_bb_loop(bool is_main_loop) {
    ...
    while(!escape_loop) {
        ...
        if(is_main_loop && rtl_link_up && (!rtl_is_copying))  // Only want this to run in main loop
        {
            ...
            // This will override set_ip_from_file() if the ip is in the
            // 0.0.0.0/8 range
            dhcp_poll();
        }
