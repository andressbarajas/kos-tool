/* client/dreamcast/net/adapter.h */
...
// Receive buffer size (max Ethernet payload: 1500 + 14-byte header)
#define RX_PKT_BUF_SIZE 1514

// Maximum Ethernet frame size on the wire (header + payload + CRC).
// Used to cap FIFO drain loops when discarding error/oversized packets —
// the hardware can't hold more than one frame, so draining beyond this
// on a corrupted length field would read garbage from an empty FIFO.
//
//| Dst MAC (6) | Src MAC (6) | EtherType (2) | Payload (46-1500) | CRC (4) |
//|------------- 14 bytes header -------------|                   |         |
//|---------------------------- 1518 bytes max -----------------------------|
