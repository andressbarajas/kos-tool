/* client/dreamcast/net/lan_adapter.c */
/*
...
int la_bb_init(void) {
    ...
    // Configure transmit mode reg to defaults (LBC normal mode (= 1), CNTRL to
    // 1) Set to half duplex, since most things use auto-negotiation now and
    // there's no way to autonegotiate 10Mbps full-duplex (detecting NLPs just
    // falls back to legacy 10Mbps half-duplex). Technically dcload does
    // half-duplex anyways, so this could be set to full-duplex and be fine, but
    // let's just do it the right way. We get error detection and the like in
    // half-duplex mode that we don't get in full-duplex, in any case.
    ...
    REGW(12) = 0;
    REGW(13) = 0;     // Don't change base address, all other params should be 0
                      // (DMA burst is single)
    REGW(14) = 0x01;  // Filter packets transmitted from self
    REGW(15) = 0;     // Writing 0 to this one does nothing (using full duplex,
                      // now, so SQE doesn't matter. Polarity is auto-corrected and
                      // LINK FAIL is checked by la_bb_loop())
...
void la_bb_stop(void) {
    ...
    /*
        while (REG(10) & 0x7f)
            net_sleep_ms(2);
    */
    while(!(REG(0) & 0x80))  // wait for any prior transmit of all packets in
                             // transmit buffer to finish
    {
        net_sleep_ms(2);
    }
    // We could reset first_transmit, or we could leave the TMT OK bit high for
...
int la_bb_tx(unsigned char *pkt, int len) {
    ...
    len &= 0x07ff;  // max length is 11 bits, meaning 4095-byte packets...but it
                    // does
    // not really matter since a standard packet maxes at a grand total of 1514
    // bytes anyways.
...
// Tx time
    ...
    if(len < 60) {
        ...
        // size allowed is 46 bytes + 14 byte ethernet header. Well, it's
        // actually 64, but the NIC is configured to auto-append a 4-byte CRC.
    } else {
        /* Poke the length */
        REGW(8) = (len & 0x00ff);
        REGW(8) = (len & 0x0700) >> 8;
...
// Tx time end
    ...
    // This will be blocked on the first transmit because setting ENA DLC
    // (REG(6) bit 0x80) clears TMT OK... So keep track of whether this is the
    // first transmit or not
    if(__builtin_expect(first_transmit,
                        0))  // if(first_transmit), but telling GCC we usually
                             // expect first_transmit to be 0
    {
        first_transmit = 0;
    } else {
        while(!(REG(0) & 0x80))  // wait for prior transmit (in the other bank,
                                 // since dcload uses dual-bank mode) to finish
        {
            net_sleep_ms(2);
        }
...
// Full loop timing
        ...
        if(__builtin_expect((status & 0x3e) != 0x20, 0)) {
            /* Drain remaining packet data from FIFO to prevent misalignment.
             * Cap at max Ethernet frame in case len is corrupted. */
            int drain = (len <= MAX_ETHERNET_FRAME_SIZE) ? len : MAX_ETHERNET_FRAME_SIZE;
            for(i = 0; i < drain; i++)
                (void)REG(8);
        ...
        if(__builtin_expect(len > RX_PKT_BUF_SIZE, 0)) {
            /* Drain oversized packet from FIFO to prevent misalignment.
             * Cap at max Ethernet frame in case len is corrupted. */
            int drain = (len <= MAX_ETHERNET_FRAME_SIZE) ? len : MAX_ETHERNET_FRAME_SIZE;
            for(i = 0; i < drain; i++)
                (void)REG(8);
...
// Rx time
        ...
        // This loop is dumb, but we are able to max out the LAN adapter with
        // it, so that's neat
        for(i = 0; i < len; i++) {
            // current_pkt[i] = REG(8);
...
void la_bb_loop(bool is_main_loop) {
    ...
    if(is_main_loop) {
        ...
        // This adapter does not support autonegotiation, so this delay ensures
        // that the other end of the link (which probably does support it) has
        // time to realize this device uses NLPs instead of FLPs. It takes
        // 50-150ms for this device to enter LINK FAIL, so wait 150ms to start
        // in link fail.
        net_sleep_ms(150);

        // In the event a user actually connects to something NOT using
        // autonegotiation, set lan_link_up to 0 so that this loop can work with
        // those, too.
        lan_link_up = 0;
    }
    ...
    while(!escape_loop) {
        ...
        if(REG(1) & 0x80)  // Do we have a packet in the receive buffer?
        {
            ...
            // Hardware will re-set this bit if there's another packet in the
            // receive buffer
        }

        // Check for ethernet cable
        // 10Base-T heartbeat signal is this chip's LINK FAIL mechanism, not the
        // "carrier sense" stuff
        if(__builtin_expect(REG(15) & 0x40, 0)) {
            /* Wake screensaver on link change so main screen is visible */
            screensaver_wake();

            if((!link_change_message) && booted && (!running) && (!receiving)) {
            ...
            // There is an excellent thread on U-Boot's mailing list about how
            // long to wait for autonegotiation to complete:
            // https://lists.denx.de/pipermail/u-boot/2009-February/047056.html
            // They ultimately use 4.5 seconds (see later in the thread) to
            // match the Linux kernel, as the Linux kernel's e1000 driver
            // defines PHY_AUTO_NEG_LIMIT as 45. The e1000 driver uses 100msec
            // sleeps in a loop that iterates PHY_AUTO_NEG_LIMIT times as a
            // timeout while checking for autonegotiation to complete (or not).
            // Well, ok, and then they go and increase it to 8 seconds:
            // https://lists.denx.de/pipermail/u-boot/2015-August/223424.html
            // But they also have a variable for 2 seconds, PHY_FORCE_TIME.
            // Since we are actually forcing a link speed, we should probably
            // thus use the 2.0 seconds described in the mailing list
            // (interestingly, it does not appear that U-Boot actually uses
            // PHY_FORCE_TIME).
        ...
        unsigned int reg_aggregate = ((unsigned int)(REG(15)) << 24) | ((unsigned int)(REG(4)) << 16) |
