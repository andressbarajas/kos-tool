...
#define DC_SYSMODE_REG         0xa05f74b0u
#define DC_SYSINFO_VECTOR      0x8c0000b0u
#define DC_SYSINFO_FUNC_INIT   0
#define DC_SYSINFO_FUNC_ID     3
#define DC_FALLBACK_ID_ADDR    0xa021a056u
...
static int dc_sysinfo_id(uint64_t *id)
{
    ...
    id_ptr = ((uint64_t *(*)(int, int, int, int))sysinfo)(
        0, 0, 0, DC_SYSINFO_FUNC_ID);
...
{
    volatile unsigned char *sysid =
        (volatile unsigned char *)DC_FALLBACK_ID_ADDR;
