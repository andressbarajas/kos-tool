...
void * SH4_mem_to_pkt_X_movca_32(void *dest, void *src, unsigned int numbytes)
{
  ...
  __asm__ volatile (
    ...
    "add #32, %[scratch_A]\n\t" // set up address in next block (EX 1) - flow dependency special case
    ...
    "mov.l @%[in]+, %[scratch_Z]\n\t" // load up first longword (leading zeros, or garbage padding, + 2 bytes of packet data) (LS 1/2)

    "add #31, %[size]\n\t" // Add extra 31 to account for all data in the last 32 bytes if size is not an exact multiple of 32 (EX 1)
    "bf.s 1f\n\t" // Branch to ensure dual-issuing is aligned as desired (BR 1/2)
    " shld %[scratch_B], %[size]\n" // integer divide numbytes by 32, -5 means shift right by 5 (2/2) (EX 1)
    // When writing to the transmit buffer, we can write some extra garbage; it's 2kB and we give the RTL a length parameter anyways.
  ".align 5\n" // Align to 32 bytes
  "1:\n\t"
    ...
    // Do this: 0xAAAA[AAAA:RRRR]RRRR -> 0xAAAARRRR with scratch_A:scratch_R0 -> scratch_R0 to interleave lower 16 bits of scratch_A with prior value
    // The result of this is equivalent to "shll16 %[scratch_A]", "shlr16 %[scratch_R0]", "extu.w %[scratch_R0]", "add %[scratch_A], %[scratch_R0]"
    // except without destroying scratch_A.
    ...
    // movca.l only writes to a cache block, whether it's read into opcache or not. So it removes the step of opcache reading.
  	// It behaves like a normal mov.l for storing to write-through/uncached memory. It also only writes data in R0...
    // We do know that output is always 32-byte aligned at this position on the bus, so we can optimize and use 1x movca.l with
    // 7x mov.l to minimize instruction count and register moves.
