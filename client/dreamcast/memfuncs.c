/* client/dreamcast/memfuncs.c */
/*
 * Copied from dcload-ip: dcload-ip/target-src/dcload/memfuncs.c
 */
...
void *memcpy_8bit(void *dest, const void *src, unsigned int len) {
  ...
  char *d = (char *)dest;
    ...
    __asm__ volatile("clrs\n"  // Align for parallelism (CO) - SH4a use "stc SR, Rn" instead
                               // with a dummy Rn
                     ".align 2\n"
                     "0:\n\t"
                     "dt %[size]\n\t"                            // (--len) ? 0 -> T : 1 -> T (EX 1)
                     ...
                     " mov.b %[scratch], @(%[offset], %[in])\n"  // *(datatype_of_s*)
                                                                 // ((char*)s + diff) =
                                                                 // scratch, where src + diff
                                                                 // = dest (LS 1)
                     : [in] "+&r"((unsigned int)s), [scratch] "=&r"((unsigned int)d),
                       [size] "+&r"(len)   // outputs
...
void *memcpy_64bit(void *dest, const void *src, unsigned int len) {
    ...
    __asm__ volatile("fschg\n\t"
                     ...
                     " fmov.d %[scratch], @(%[offset], %[in])\n\t"  // *(datatype_of_s*)
                                                                    ...
                                                                    // diff = dest (LS 1)
                     "fschg\n"
                     : [in] "+&r"((unsigned int)s), [scratch] "=&d"(double_scratch),
                       [size] "+&r"(len)   // outputs
                     : [offset] "z"(diff)  // inputs
                     : "t", "memory"       // clobbers
    );
...
void *memcpy_64bit_32Bytes(void *dest, const void *src, unsigned int len) {
    ...
    __asm__ volatile("fschg\n\t"  // Switch to pair move mode (FE)
                     ...
                     : [in] "+&r"((unsigned int)src), [out] "+&r"((unsigned int)dest), [size] "+&r"(len),
                       ...
                       [scratch3] "=&d"(double_scratch3),
                       [scratch4] "=&d"(double_scratch4)  // outputs
                     :                                    // inputs
                     : "t", "memory"                      // clobbers
    );
...
void *memset_zeroes_64bit(void *dest, unsigned int len) {
    ...
    __asm__ volatile("fldi0 fr0\n\t"
                     "fldi0 fr1\n\t"
                     "fschg\n\t"       // Switch to pair move mode (FE)
                     "dt %[size]\n\t"  // Decrement and test size here once to prevent extra
                                       // jump (EX 1)
                     "clrs\n"          // Align for parallelism (CO) - SH4a use "stc SR, Rn" instead
                                       // with a dummy Rn
                     ".align 2\n"
...
void *SH4_mem_to_pkt_X_movca_32(void *dest, void *src, unsigned int numbytes) {
    ...
    __asm__ volatile(
        ...
        "add #32, %[scratch_A]\n\t"    // set up address in next block (EX 1) -
                                       // flow dependency special case
        ...
        "mov #-5, %[scratch_B]\n\t"        // integer divide numbytes by 32 (1/2) (EX
                                           // 1)
        "mov.l @%[in]+, %[scratch_Z]\n\t"  // load up first longword (leading
                                           // zeros, or garbage padding, + 2
                                           // bytes of packet data) (LS 1/2)

        "add #31, %[size]\n\t"           // Add extra 31 to account for all data in the
                                         // last 32 bytes if size is not an exact
                                         // multiple of 32 (EX 1)
        "bf.s 1f\n\t"                    // Branch to ensure dual-issuing is aligned as desired
                                         // (BR 1/2)
        " shld %[scratch_B], %[size]\n"  // integer divide numbytes by 32, -5
                                         // means shift right by 5 (2/2) (EX 1)
        // When writing to the transmit buffer, we can write some extra garbage;
        // it's 2kB and we give the RTL a length parameter anyways.
        ".align 5\n"  // Align to 32 bytes
        "1:\n\t"
        ...
        "mov %[scratch_Z], %[scratch_R0]\n\t"  // Copy scratch_Z into scratch_R0
                                               // for next pass (MT 0)

        "mov.l @%[in]+, %[scratch_C]\n\t"  // Read 32-bit int C (LS 1/2)
        // Do this: 0xAAAA[AAAA:RRRR]RRRR -> 0xAAAARRRR with
        // scratch_A:scratch_R0 -> scratch_R0 to interleave lower 16 bits of
        // scratch_A with prior value The result of this is equivalent to
        // "shll16 %[scratch_A]", "shlr16 %[scratch_R0]", "extu.w
        // %[scratch_R0]", "add %[scratch_A], %[scratch_R0]" except without
        // destroying scratch_A.
        ...
        // movca.l only writes to a cache block, whether it's read into opcache
        // or not. So it removes the step of opcache reading. It behaves like a
        // normal mov.l for storing to write-through/uncached memory. It also
        // only writes data in R0... We do know that output is always 32-byte
        // aligned at this position on the bus, so we can optimize and use 1x
        // movca.l with 7x mov.l to minimize instruction count and register
        // moves.
        "movca.l %[scratch_R0], @%[out]\n\t"    // Write data to out 0 R0 (LS 3-7)
        "xtrct %[scratch_Y], %[scratch_X]\n\t"  // (EX 1)

        "mov %[in], %[scratch_R0]\n\t"  // Reuse R0 - To make base of next cache
                                        // block (MT 0)
        "add #28, %[scratch_R0]\n\t"    // Reuse R0 (EX 1) -- flow dependency
                                        // special case

        "pref @%[scratch_R0]\n\t"               // Reuse R0 - To prefetch the next src block
                                                // here (LS 1)
        ...
        "mov %[out2], %[scratch_R0]\n\t"  // Reuse R0 - Need to purge the cache
                                          // block just written to because
                                          // 0x81848000 is volatile and used for
                                          // both reads and writes (MT 0)
        "add #-28, %[scratch_R0]\n\t"     // Reuse R0 (EX 1) -- flow dependency
                                          // special case

        "ocbp @%[scratch_R0]\n\t"  // Reuse R0 (LS 1-5)
        "bf.s 1b\n\t"              // (BR 1/2)
        " add #8, %[out2]\n"       // out2 is now 32 from prev out2 and offset by 4
                                   // from out (EX 1)

        : [in] "+&r"((unsigned int)s), [out] "+&r"((unsigned int)d), [scratch_R0] "=&z"(scratch_reg),
          [scratch_A] "=&r"(scratch_reg2), [scratch_B] "=&r"(scratch_reg3), [scratch_C] "=&r"(scratch_reg4),
          [scratch_D] "=&r"(scratch_reg5), [scratch_W] "=&r"(scratch_reg6), [scratch_X] "=&r"(scratch_reg7),
          [scratch_Y] "=&r"(scratch_reg8), [scratch_Z] "=&r"(scratch_reg9), [out2] "=&r"(scratch_reg10),
          [size] "+&r"(numbytes)
        :                // inputs
        : "t", "memory"  // clobbers
    );
