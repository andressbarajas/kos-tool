// ---- client/dreamcast/perfctr.c - SH7750/SH7091 Performance Counter Module
// Code ----
//
...
void PMCR_Init(unsigned char which, unsigned char mode,
               unsigned char count_type)  // Will do nothing if perfcounter is
                                          // already running!
{
...
// Enable "undocumented" performance counters (well, they were undocumented at
// one point. They're documented now!)
void PMCR_Enable(unsigned char which, unsigned char mode, unsigned char count_type,
                 unsigned char reset_count)  // Will do nothing if perfcounter
                                             // is already running!
{
    ...
    unsigned short pmcr_ctrl = PMCR_RUN_COUNTER | (reset_count << PMCR_RESET_COUNTER_SHIFT) |
                               (count_type << PMCR_CLOCK_TYPE_SHIFT) | mode;
...
// NOTE: It does not appear to be possible to clear a counter while it is
// running.
...
void PMCR_Read(unsigned char which, volatile unsigned int *out_array) {
    ...
    // also portable between versions of GCC that do put the nonsensical branch
    // in.
    //
    // One thing that would be nice is if SH4 had the movi20s instruction to
    // make absolute addresses in 3 cycles, but only the SH2A has that... :(
    if(which == 1) {
        // counter 1
        //		out_array[1] = *((volatile unsigned int*)PMCTR1H_REG) & 0xffff;
        //		out_array[0] = *((volatile unsigned int*)PMCTR1L_REG);
        __asm__ volatile("mov.l @%[reg1h],%[reg1h]\n\t"  // read counter (high)
                         "mov.l @%[reg1l],%[reg1l]\n\t"  // read counter (low)
                         "extu.w %[reg1h],%[reg1h]\n\t"  // zero-extend high, aka high &
                                                         // 0xffff
                         "mov.l %[reg1h],%[outh]\n\t"    // get data to memory
                         "mov.l %[reg1l],%[outl]\n\t"    // get data to memory
                         : [outh] "=m"(out_array[1]), [outl] "=m"(out_array[0]), [reg1h] "+&r"(pmcr1_regh),
                           [reg1l] "+r"(pmcr1_regl)  // SH4 can't mov an immediate longword
                                                     // into a register...
                         :                           // no inputs
                         :                           // no clobbers
        );
        ...
        __asm__ volatile("mov.l @%[reg2h],%[reg2h]\n\t"  // read counter (high)
                         "mov.l @%[reg2l],%[reg2l]\n\t"  // read counter (low)
                         "extu.w %[reg2h],%[reg2h]\n\t"  // zero-extend high, aka high &
                                                         // 0xffff
                         "mov.l %[reg2h],%[outh]\n\t"    // get data to memory
                         "mov.l %[reg2l],%[outl]\n\t"    // get data to memory
                         : [outh] "=m"(out_array[1]), [outl] "=m"(out_array[0]), [reg2h] "+&r"(pmcr2_regh),
                           [reg2l] "+r"(pmcr2_regl)  // SH4 can't mov an immediate longword
                                                     // into a register...
                         :                           // no inputs
                         :                           // no clobbers
        );
...
unsigned long long int PMCR_RegRead(unsigned char which) {
    ...
    if(which == 1) {
        ...
        // counter 1
        // output value = (unsigned long long int)(*((volatile unsigned
        // int*)PMCTR1H_REG) & 0xffff) << 32 | (unsigned long long
        // int)(*((volatile unsigned int*)PMCTR1L_REG));
        __asm__ volatile("mov.l @%[reg1h],%[reg1h]\n\t"  // read counter (high)
                         "mov.l @%[reg1l],%[reg1l]\n\t"  // read counter (low)
        ...
        // counter 2
        // output value = (unsigned long long int)(*((volatile unsigned
        // int*)PMCTR2H_REG) & 0xffff) << 32 | (unsigned long long
        // int)(*((volatile unsigned int*)PMCTR2L_REG));
        __asm__ volatile("mov.l @%[reg2h],%[reg2h]\n\t"  // read counter (high)
                         "mov.l @%[reg2l],%[reg2l]\n\t"  // read counter (low)
                         "extu.w %[reg2h],%[reg2h]\n"    // zero-extend high, aka high & 0xffff
...
// Convert performance counter ticks to seconds using SH4 FPU
// double-precision division by CPU frequency (199496956 Hz).
//
// SH4_FREQUENCY as IEEE 754 double: 0x41a7c829_f8000000
// 2^32 as IEEE 754 double:          0x41f00000_00000000

__attribute__((aligned(8))) static const unsigned int const_perf_freq[2] = {0x41a7c829, 0xf8000000};
__attribute__((aligned(8))) static const unsigned int const_32[2] = {0x41f00000, 0x00000000};

unsigned int PMCR_TicksToSeconds(unsigned long long int ticks) {
    unsigned int result = 0;
    unsigned int old_fpscr = 0;

    /* Save fpscr */
    __asm__ volatile("sts fpscr,r1\n\t"
    ...
    unsigned int double_tmp_fpscr = old_fpscr | (1 << 19); /* FPSCR.PR = 1 */
    double_tmp_fpscr &= ~(1 << 20);                        /* FPSCR.SZ = 0 */

    __asm__ volatile("mov %[in_dbl_tmp],r1\n\t"
                     : /* no outputs */
                     : [in_dbl_tmp] "r"(double_tmp_fpscr)
                     : "r1");
    __asm__ volatile("lds r1,fpscr\n\t");

    /* Load constants into FPU registers */
    __asm__ volatile("fmov.s @%[in_dblfreq]+,FR2\n\t"
    ...
    /* Convert 48-bit int to double and divide by frequency */
    __asm__ volatile("lds %[in_dbl0],FPUL\n\t"
                     "cmp/pl %[in_dbl0]\n\t"
                     "bt.s 1f\n\t"
                     "float FPUL,DR0\n\t"
                     "fadd DR6,DR0\n\t"
                     ...
                     "sts FPUL,%[out_int]\n\t"
                     : [out_int] "=r"(result)
                     : [in_dbl0] "r"(((unsigned int *)&ticks)[0]), [in_dbl1] "r"(((unsigned int *)&ticks)[1])
                     : "t", "fpul", "dr0", "dr2", "dr4", "dr6");

    /* Restore fpscr */
    __asm__ volatile("mov %[in_old_fpscr],r1\n\t"
                     : /* no outputs */
