...
void cmd_capabilities(ip_header_t *ip, udp_header_t *udp, command_t *command) {
    ...
    make_ip(ntohl(ip->src), our_ip, UDP_H_LEN + COMMAND_LEN, IP_UDP_PROTOCOL,
            (ip_header_t *)(pkt_buf + ETHER_H_LEN), ip->packet_id);
    make_udp(ntohs(udp->src), kosload_syscall_port, COMMAND_LEN, (ip_header_t *)(pkt_buf + ETHER_H_LEN),
             (udp_header_t *)(pkt_buf + ETHER_H_LEN + IP_H_LEN));
    bb->tx(pkt_buf, ETHER_H_LEN + IP_H_LEN + UDP_H_LEN + COMMAND_LEN);
}
...
void cmd_retval(ip_header_t *ip, udp_header_t *udp, command_t *command) {
    if(running) {
        ...
        if(tool_version >= 0x00030000 && udp_payload_len >= COMMAND_LEN + NET_SEQ_TRAILER_LEN) {
            trailer = (unsigned char *)command + udp_payload_len - NET_SEQ_TRAILER_LEN;
            if(memcmp(trailer, NET_SEQ_MAGIC, NET_SEQ_MAGIC_LEN) == 0) {
                unsigned int echoed = ((unsigned int)trailer[NET_SEQ_MAGIC_LEN + 0] << 24) |
        ...
        data_len = (udp_payload_len > COMMAND_LEN) ? udp_payload_len - COMMAND_LEN : 0;
