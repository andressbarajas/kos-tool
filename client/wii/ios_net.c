...
static const uint8_t fallback_mac[6] = {0x02, 0x57, 0x49, 0x49, 0x00, 0x01};
...
static uint32_t      ioctl_words_storage[IOCTL_WORDS_SIZE / sizeof(uint32_t)] __attribute__((aligned(32)));
static ios_iovec_t   ioctl_vectors_storage[4] __attribute__((aligned(32)));
static uint8_t       sockaddr_buf_storage[SOCKADDR_BUF_SIZE] __attribute__((aligned(32)));
static uint8_t       sockaddr_len_buf_storage[SOCKADDR_LEN_BUF_SIZE] __attribute__((aligned(32)));
static uint8_t       meta_buf_storage[META_BUF_SIZE] __attribute__((aligned(32)));
static uint8_t       rx_buf_storage[WII_NET_MAX_UDP_PAYLOAD] __attribute__((aligned(32)));
static uint8_t       tx_buf_storage[WII_NET_MAX_UDP_PAYLOAD] __attribute__((aligned(32)));
...
#define MEM2_MAC_BUFS_BASE 0x90100000
...
static int ipc_sync(ios_request_t *req) {
    ...
    for(;;) {
        ...
        if(!ipc_block && (uint32_t)(ticks() - start) > (IPC_TIMEOUT_US * 61)) {
            ipc_timeout_valid = 1;
...
static int ios_ioctl(int fd, uint32_t cmd, void *in, uint32_t in_len, void *out, uint32_t out_len) {
    request_clear();
...
static int ios_ioctlv(int fd, uint32_t cmd, int in_count, int out_count, ios_iovec_t *vec) {
    void *orig[8];
...
static int ncd_ioctlv_out2(int fd, uint32_t cmd, void *out0, uint32_t out0_len, void *out1,
                           uint32_t out1_len) {
...
static void ncd_parse_config(void) {
    ...
    for(i = 0; i < (int)NCD_CONNECTION_COUNT; i++) {
        const uint8_t *entry = ncd_config + NCD_HEADER_SIZE + (uint32_t)i * NCD_CONNECTION_SIZE;
    ...
    {
        const uint8_t *entry = ncd_config + NCD_HEADER_SIZE + (uint32_t)selected * NCD_CONNECTION_SIZE;
        uint8_t        flags = entry[NCD_CONN_FLAGS];

        ncd_ip_source_dhcp = ((flags & NCD_FLAG_IP_DHCP) != 0) ? 1 : 0;
        ncd_interface_type =
            ((flags & NCD_FLAG_INTERFACE) != 0) ? WII_NET_INTERFACE_WIRED : WII_NET_INTERFACE_WIFI;
    }
}
...
static void ncd_probe_wifi_mac(void) {
    ...
    ret = ncd_ioctlv_out2(fd, NCD_CMD_GETWIFIMAC, ncd_out, NCD_OUT_SIZE, ncd_mac_buf, 6);
    ios_close(fd);
...
static void ncd_probe_config(int ncd_fd) {
    ...
    ret = ncd_ioctlv_out2(ncd_fd, NCD_CMD_GETIFCONFIG, ncd_config, NCD_CONFIG_SIZE, ncd_out, NCD_OUT_SIZE);
    if(ret < 0) {
        memset(ncd_config, 0, NCD_CONFIG_SIZE);
        ret = ncd_ioctlv_out2(ncd_fd, NCD_CMD_READCONFIG, ncd_config, NCD_CONFIG_SIZE, ncd_out, NCD_OUT_SIZE);
    }
...
static void choose_display_mac(void) {
    ...
    if(ncd_interface_type == WII_NET_INTERFACE_WIRED && ethernet_mac_valid) {
...
static int wii_ios_net_getifopt(uint32_t optname, uint8_t *out, uint32_t outlen) {
    ...
    ioctl_vectors[0].data = ifopt_req_buf;
    ioctl_vectors[0].len = 8;
    ioctl_vectors[1].data = ifopt_val_buf;
    ioctl_vectors[1].len = outlen;
    ioctl_vectors[2].data = ifopt_len_buf;
    ioctl_vectors[2].len = 4;
