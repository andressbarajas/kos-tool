 ...
 * I2C bus on Hollywood GPIO (SCL=0x4000, SDA=0x8000 of HW_GPIO1B at
 * 0xCD8000C0). A System-Menu channel launch leaves the encoder unconfigured, so
 * VI timing alone yields no signal (black) — unlike GameCube, whose encoder the
 * IPL sets up.  This programs the encoder for NTSC 480i.  Protocol, register
 * map and the linear-gamma table are from the WiiBrew "AV Encoder" RE notes
 * (clean-room documentation, not SDK source), cross-checked against the
 * Homebrew Channel binary which does the same I2C writes. */
...
#define AVE_SCL      0x4000
#define AVE_SDA      0x8000
#define AVE_ADDR     0xe0 /* 0x70<<1 | W  */

static void ave_delay(void) {
    unsigned int a, b;
    __asm__ volatile("mftb %0" : "=r"(a));
    do {
        __asm__ volatile("mftb %0" : "=r"(b));
    } while((b - a) < 300); /* ~5us */
}
static void ave_scl(int v) {
    if(v)
        AVE_GPIO_OUT |= AVE_SCL;
    else
        AVE_GPIO_OUT &= ~AVE_SCL;
    __asm__ volatile("eieio");
}

static void ave_sda(int v) {
    if(v)
        AVE_GPIO_OUT |= AVE_SDA;
    else
        AVE_GPIO_OUT &= ~AVE_SDA;
    __asm__ volatile("eieio");
}

static void ave_sda_out(void) {
    AVE_GPIO_DIR |= AVE_SDA;
    __asm__ volatile("eieio");
}

static void ave_sda_in(void) {
    AVE_GPIO_DIR &= ~AVE_SDA;
    __asm__ volatile("eieio");
}

static int ave_sda_read(void) {
    return (AVE_GPIO_IN & AVE_SDA) ? 1 : 0;
}
...
static void wii_ave_init(void) {
    ...
    AVE_GPIO_DIR |= (AVE_SCL | AVE_SDA);
    __asm__ volatile("eieio"); /* SCL,SDA outputs */
    AVE_GPIO_OUT |= (AVE_SCL | AVE_SDA);
    __asm__ volatile("eieio"); /* idle high       */

    ave_w(0x6a, 0x01);
    ave_w(0x65, 0x03); /* HBC value (was 0x01 in WiiBrew docs) */
    ave_w(0x00, 0x00); /* A/V timings -- HBC does NOT write 0x01 */
    ave_w(0x71, 0x8e);
    ave_w(0x72, 0x8e); /* audio volume */
    ave_w(0x02, 0x07); /* VBI */
    ave_w(0x05, 0x00);
    ave_w(0x06, 0x00); /* CGMS */
    ave_w(0x08, 0x00);
    ave_w(0x09, 0x00); /* WSS */
    ave_w(0x7a, 0x00);
    ave_w(0x7b, 0x00); /* HBC adds these four */
    ave_w(0x7c, 0x00);
    ave_w(0x7d, 0x00);
    for(i = 0x40; i <= 0x59; i++)
        ave_w((unsigned char)i, 0x00); /* macrovision off */
    ave_w(0x0a, 0x00);                 /* HBC adds this */
    ave_w(0x03, 0x01);                 /* composite trap filter */
    for(i = 0; i < 33; i++)
        ave_w((unsigned char)(0x10 + i), gamma[i]);
...
static void wii_set_console_enabled(bool enabled) {
    ...
    *(volatile unsigned int *)(WII_LOADER_BASE + 4) = enabled ? 0xdeadbeef : 0xfeedface;
}
...
#define WII_TBR_TICKS_PER_SEC 60750000
...
static void wii_fill_rect(int x, int y, int w, int h, uint32_t color) {
    ...
    for(row = 0; row < h; row++) {
        uint32_t *fb32 = (uint32_t *)(XFB_ADDR + (y + row) * GC_SCREEN_WIDTH * 2 + (x & ~1) * 2);
        int pairs = (w + 1) / 2;
        int i;
        for(i = 0; i < pairs; i++)
            fb32[i] = fill;
...
static void wii_draw_bitmap(int x, int y, int w, int h, const uint32_t *bits, uint32_t color) {
    ...
    for(row = 0; row < h; row++) {
        ...
        for (col = 0; col < w; col += 2) {
            ...
            fb32 = (uint32_t *)(XFB_ADDR + (y + row) * GC_SCREEN_WIDTH * 2 + ((x + col) & ~1) * 2);
            if(p0 && p1) {
                *fb32 =
                    ((uint32_t)c_y << 24) | ((uint32_t)c_cb << 16) | ((uint32_t)c_y << 8) | (uint32_t)c_cr;
            } else {
