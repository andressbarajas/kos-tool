 ...
 */
static unsigned int *maple_cmd(int port, int unit, int cmd, int datalen, const unsigned int *data) {
    ...
    *sendbuf++ = (unsigned int)(cmd & 0xff) | ((unsigned int)to << 8) | ((unsigned int)from << 16) |
                 ((unsigned int)datalen << 24);
...
void start(void) {
    ...
    for(int port = 0; port < 4; port++) {
        ...
        /* Test GETCOND if it's a controller (datalen=1, exercises datalen=1
         * path) */
        ...
        /* Test BREAD if it has a memory card function (datalen=2, exercises the
         * fix) */
        if(info->func & MAPLE_FUNC_MEMCARD) {
            ...
            for(int unit = 1; unit <= 2; unit++) {
                ...
                if(!(sub_info->func & MAPLE_FUNC_MEMCARD))
                    continue;
                 ...
                 * This is the code path that was broken before the
                 * (datalen-1)*4 fix */
                ...
                if(resp_code == MAPLE_RESP_DATATRF) {
                    ...
                    result("BREAD block 0 (datalen=2)", resp_code == (unsigned char)(-5 & 0xff));
                }
