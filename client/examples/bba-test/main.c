...
#define BBA_CMD_RD  0x80000000
#define BBA_CMD_WR  0xC0000000
#define BBA_CMD(dir, addr) ((dir) | (((addr) & 0xFFFF) << 8))
...
static void bba_sel(void) {
    exi_select(BBA_CH, BBA_DEV, CLK_32MHZ);
}

static void bba_desel(void) {
    exi_deselect(BBA_CH);
}
...
static void bba_cport_w2(unsigned char op, unsigned char d0, unsigned char d1) {
    bba_sel();
    exi_imm(BBA_CH, (unsigned int)op << 24, 2, IMM_WRITE);
    exi_imm(BBA_CH, ((unsigned int)d0 << 24) | ((unsigned int)d1 << 16), 2, IMM_WRITE);
    bba_desel();
}
...
static int bba_init(void) {
    ...
    /* 9. Buffer pointers */
    bba_out8(R_TLBP, 0x00);
    bba_out8(R_TLBP + 1, 0x00);
    bba_out8(R_BP, 0x01);
    bba_out8(R_BP + 1, 0x00);
    bba_out8(R_RHBP, 0x10);
    bba_out8(R_RHBP + 1, 0x00);
    bba_out8(R_RWP, 0x01);
    bba_out8(R_RWP + 1, 0x00);
    bba_out8(R_RRP, 0x01);
    bba_out8(R_RRP + 1, 0x00);
...
static void decode_nways(unsigned char nways) {
    ...
    if(nways & NWAYS_LS100)
        print(" LS100");
    if(nways & NWAYS_LPNWAY)
        print(" LPNWAY");
    if(nways & NWAYS_ANCLPT)
        print(" ANCLPT");
    if(nways & NWAYS_100TXF)
        print(" 100TXF");
    if(nways & NWAYS_100TXH)
        print(" 100TXH");
    if(nways & NWAYS_10TXF)
        print(" 10TXF");
    if(nways & NWAYS_10TXH)
        print(" 10TXH");
}
...
static void loader_init_test(void) {
    ...
    unsigned char preserve = bba_in8(R_NWAYC) & 0xC0;
    unsigned char target = preserve | NWAYC_FD | NWAYC_PS100_10 | NWAYC_ANE | NWAYC_ANS_RA;
...
static void test_tx_speed(int pkt_size, int count) {
    ...
    int wait = 100000;
    while(wait > 0 && (bba_in8(R_NCRA) & NCRA_ST1))
        wait--;
    bba_out8(R_IR, IR_TI);
    ...
    for(i = 0; i < count; i++) {
        ...
        wait = 100000;
        while(wait > 0 && !(bba_in8(R_IR) & IR_TI))
            wait--;
        bba_out8(R_IR, IR_TI);
    }
