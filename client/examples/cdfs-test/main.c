...
static void kl_write(int fd, const void *buf, int count) {
    ...
    if(sc)
        sc(SYSCALL_WRITE, fd, (int)buf, count);
}
...
void start(void) {
    ...
    if(ret == 2) {
        ...
        result("Test 3: TOC entry[0] = 0x41000096", toc.entry[0] == 0x41000096);
        result("Test 4: TOC first = last = 0x41010000", toc.first == 0x41010000 && toc.last == 0x41010000);
    ...
    if(ret == 2) {
        ...
        int cd001_ok = (sector_buf[1] == 'C' && sector_buf[2] == 'D' && sector_buf[3] == '0' &&
                        sector_buf[4] == '0' && sector_buf[5] == '1');
