...
#define TB_TICKS_PER_MS  40500u
#define TB_TICKS_PER_US  41u
...
static void enc_raw_clear_bank_bits(void)
{
    enc_select();
    exi_imm(ENC_CH,
            ENC_CMD_BFC(ECON1) | ((u32)(ECON1_BSEL0 | ECON1_BSEL1) << 16),
            2, IMM_WRITE);
    enc_deselect();
    current_bank = 0;
}
...
static int enc_phy_id_ok(u16 *phid1, u16 *phid2)
{
    ...
    if (!enc_phy_read(PHID2, phid2))
        return 0;
    return *phid1 == ENC28J60_PHID1_EXPECTED &&
...
static void dump_snapshot(const char *label)
{
    ...
    if (enc_phy_read(PHID1, &phid1) && enc_phy_read(PHID2, &phid2) &&
        enc_phy_read(PHCON1, &phcon1) && enc_phy_read(PHCON2, &phcon2) &&
        enc_phy_read(PHSTAT2, &phstat2)) {
...
static int enc_driver_like_init(int *link_up)
{
    ...
    enc_clear_bits(EIR, EIR_RXERIF | EIR_TXERIF | EIR_TXIF |
...
static void test_dirty_id_reset(void)
{
    ...
    result("wake reset restores EXI ID after dirty ERDPT",
           restore_ok && id_reset == ENC28J60_EXI_ID);
}
...
static void test_power_save_reset(void)
{
    ...
    result("wake reset recovers PHY after PWRSV",
           recover_ok && restore_ok);
}
...
static void test_phy_cleanup(void)
{
    ...
    result("driver-like init disables half-duplex loopback",
           init_ok && ((phcon2 & PHCON2_HDLDIS) != 0));
    enc_stop_runtime();
}
...
static void test_reinit_cycles(void)
{
    ...
    for (cycle = 0; cycle < 6; cycle++) {
        ...
        if (!init_ok ||
            (econ1_live & ECON1_RXEN) == 0 ||
            (econ1_live & (ECON1_TXRTS | ECON1_TXRST | ECON1_RXRST)) != 0 ||
            (econ2_live & ECON2_PWRSV) != 0 ||
    ...
    if (any_link)
        ...
        print("[WARN] link was never observed; check cable/router if unexpected\n");
}
