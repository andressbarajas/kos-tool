...
const char *discover_network_device(void) {
    ...
    for(int attempt = 0; attempt < 4; attempt++) {
        for(int p = 0; p < 2; p++) {
            ...
            sendto(sock, (const char *)cmd, 12, 0, (struct sockaddr *)&bcast, sizeof(bcast));
        }
        ...
        for(;;) {
            ...
            int rv = recvfrom(sock, (char *)buffer, sizeof(buffer), 0, (struct sockaddr *)&from, &fromlen);
    ...
    /* Phase 2: subnet scan (Windows) */
    {
        ...
        ULONG ret = GetAdaptersAddresses(
            AF_INET, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER, NULL, addrs,
            &buf_size);
        if(ret == ERROR_BUFFER_OVERFLOW) {
            addrs = (PIP_ADAPTER_ADDRESSES)realloc(addrs, buf_size);
            ret = GetAdaptersAddresses(
                AF_INET, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER, NULL,
                addrs, &buf_size);
        }
        if(ret != NO_ERROR) {
            free(addrs);
            ...
            for(PIP_ADAPTER_UNICAST_ADDRESS ua = aa->FirstUnicastAddress; ua; ua = ua->Next) {
                ...
                for(uint32_t h = 1; h < host_mask; h++) {
                    ...
                    for(int p = 0; p < 2; p++) {
                        ...
                        sendto(sock, (const char *)cmd, 12, 0, (struct sockaddr *)&target, sizeof(target));
                    }
                }
            }
        }
...
cleanup:
    ...
    for(int attempt = 0; attempt < 4; attempt++) {
        for(int p = 0; p < 2; p++) {
            bcast.sin_port = htons(ports[p]);
            sendto(sock, cmd, 12, 0, (struct sockaddr *)&bcast, sizeof(bcast));
        }
        ...
        for(;;) {
            ...
            ssize_t rv = recvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr *)&from, &fromlen);
            if(rv >= 12 && memcmp(buffer, NET_CMD_VERSION, 4) == 0 &&
               is_valid_unicast(from.sin_addr.s_addr)) {
                close(sock);
                inet_ntop(AF_INET, &from.sin_addr, ip_str, sizeof(ip_str));
            ...
            uint64_t elapsed =
                (uint64_t)(now.tv_sec - start.tv_sec) * 1000000 + (now.tv_usec - start.tv_usec);
            if(elapsed >= 500000)
                break;
        }
    }
        ...
        if(getifaddrs(&ifap) == 0) {
            for(struct ifaddrs *ifa = ifap; ifa; ifa = ifa->ifa_next) {
                ...
                for(uint32_t h = 1; h < host_mask; h++) {
                    ...
                    for(int p = 0; p < 2; p++) {
                        target.sin_port = htons(ports[p]);
                        sendto(sock, cmd, 12, 0, (struct sockaddr *)&target, sizeof(target));
                    }
                }
            }
            freeifaddrs(ifap);
