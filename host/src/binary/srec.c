 ...
 */
static int srec_load(const char *filename, uint32_t *entry_addr,
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        int type = line[1] - '0';
        if (type < 1 || type > 3) continue;   /* only data records */
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (type >= 1 && type <= 3) {
            ...
            for (int i = 0; i < byte_count; i++) {
                ...
                if (b < 0) { free(data); fclose(f); return -1; }
                sum += (uint8_t)b;
            }
            int cksum = hex_byte(&line[2 + byte_count * 2]);
            if (cksum < 0) { free(data); fclose(f); return -1; }
            sum += (uint8_t)cksum;
            if (sum != 0xFF) {
