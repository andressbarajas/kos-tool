...
static void load_input(void) {
    ...
    if (!f) die("cannot open input %s", g_in_name);
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    if (sz < 52) die("input too small to be ELF (%ld bytes)", sz);
    fseek(f, 0, SEEK_SET);
    g_in = (uint8_t *)malloc((size_t)sz);
    if (!g_in) die("oom %ld bytes", sz);
...
static void parse_elf_header(void) {
    ...
    if (g_e_type != ET_REL && g_e_type != ELF_IRX_TYPE)
        die("not relocatable (e_type=%u, expected ET_REL=1 or ELF_IRX_TYPE=0xFF80)",
            g_e_type);
    if (g_e_shentsize != 40)
...
static void parse_section_headers(void) {
    ...
    for (int i = 0; i < g_e_shnum; i++) {
        struct Section *s = &g_sections[i];
        if (strcmp(s->name, ".iopmod") == 0) g_iopmod_idx = i;
        if (s->sh_type == SHT_SYMTAB)        g_symtab_idx = i;
    }
    if (g_iopmod_idx < 0) {
...
static void mark_sections_to_keep(void) {
    ...
    for (int i = 0; i < g_e_shnum; i++) {
        ...
        if (s->sh_info >= (uint32_t)g_e_shnum) continue;
...
static void sort_relocations(void) {
    for (int i = 0; i < g_e_shnum; i++) {
        struct Section *s = &g_sections[i];
        if (!s->keep || s->sh_type != SHT_REL) continue;
        ...
        for (uint32_t j = 1; j < n; j++) {
            ...
            while (k >= 0) {
                ...
                if (off_k <= off_j) break;
                memcpy(base + ((uint32_t)k + 1) * 8,
                       base + (uint32_t)k * 8, 8);
                k--;
            }
    ...
    for (int i = 0; i < g_e_shnum; i++) {
        ...
        for (size_t k = 0; k < n; k++) {
            ...
            case R_MIPS_GPREL16:
                break;
            default:
                die("unsupported relocation type %u in %s entry %zu",
                    type, s->name, k);
            }
        }
    }
...
static uint32_t section_stray_count(int rel_sec_idx) {
    ...
    for (uint32_t i = 0; i < g_stray_count; i++)
        if (g_strays[i].rel_sec_idx == rel_sec_idx) count++;
    return count;
}
...
static uint32_t section_extra_lo_count(int rel_sec_idx) {
    ...
    for (uint32_t i = 0; i < g_extra_lo_count; i++)
        if (g_extra_los[i].rel_sec_idx == rel_sec_idx) count++;
    return count;
}
...
static uint32_t output_relocation_size(const struct Section *s) {
    ...
    return (kept + section_stray_count(rel_sec_idx) +
            section_extra_lo_count(rel_sec_idx)) * 8;
}
...
static void detect_stray_hi16(void) {
    for (int i = 0; i < g_e_shnum; i++) {
        struct Section *s = &g_sections[i];
        if (!s->keep || s->sh_type != SHT_REL) continue;
        if (s->sh_info >= g_e_shnum) continue;
        ...
        if (!lo_user) die("oom for lo_user");
        for (uint32_t k = 0; k < n; k++) lo_user[k] = -1;

        for (uint32_t k = 0; k < n; k++) {
            uint32_t info_k = rd32(src + k * 8 + 4);
            if (ELF32_R_TYPE(info_k) != R_MIPS_HI16) continue;
            uint32_t sym_k = ELF32_R_SYM(info_k);
            int m = -1;
            for (uint32_t j = k + 1; j < n; j++) {
                uint32_t info_j = rd32(src + j * 8 + 4);
                if (ELF32_R_TYPE(info_j) != R_MIPS_LO16) continue;
                if (ELF32_R_SYM(info_j) != sym_k) continue;
                m = (int)j;
                break;
            }
            if (m < 0) continue;  /* stray-no-match handled later */
            ...
            if (lo_user[m] >= 0) {
                if (g_stray_count == g_stray_capacity) {
                    ...
                    if (!g_strays) die("oom for strays");
                }
    ...
    if (g_verbose && g_stray_count > 0) {
        ...
        for (uint32_t i = 0; i < g_stray_count; i++) {
            ...
            fprintf(stderr, "  [%u] section %s offset 0x%x (paired LO16 at 0x%x)\n",
                    i, g_sections[sh->rel_sec_idx].name,
                    sh->hi16_off_in, sh->lo16_off_in);
        }
    }
}
...
static void detect_extra_lo16(void) {
    for (int i = 0; i < g_e_shnum; i++) {
        ...
        if (!hi_has_lo) die("oom for hi_has_lo");

        for (uint32_t k = 0; k < n; k++) {
            uint32_t info_k = rd32(src + k * 8 + 4);
            if (ELF32_R_TYPE(info_k) != R_MIPS_LO16) continue;
            uint32_t sym_k = ELF32_R_SYM(info_k);

            int hi = -1;
            for (int j = (int)k - 1; j >= 0; j--) {
                uint32_t info_j = rd32(src + (uint32_t)j * 8 + 4);
                if (ELF32_R_TYPE(info_j) != R_MIPS_HI16) continue;
                if (ELF32_R_SYM(info_j) != sym_k) continue;
                hi = j;
                break;
            }
            ...
            if (g_extra_lo_count == g_extra_lo_capacity) {
                ...
                g_extra_los = realloc(g_extra_los,
                                      g_extra_lo_capacity * sizeof(*g_extra_los));
                if (!g_extra_los) die("oom for extra LO16 records");
            }
    ...
    if (g_verbose && g_extra_lo_count > 0) {
        ...
        for (uint32_t i = 0; i < g_extra_lo_count; i++) {
            ...
            fprintf(stderr, "  [%u] section %s LO16 index %u duplicates HI16 at 0x%x\n",
                    i, g_sections[el->rel_sec_idx].name,
                    el->lo16_in_idx, el->hi16_off_in);
        }
    }
}
...
static void compute_layout(void) {
    ...
    if (scratch_reloc_word_count() > 0) {
        ...
        file_top = padlo_end;
        if (mem_top < padlo_end) mem_top = padlo_end;
        else                     mem_top += shift;
    }

    g_out_filesz = (file_top + 3) & ~3u;
    g_out_memsz  = (mem_top   + 3) & ~3u;
    ...
    g_out_alloc_off  = (g_out_iopmod_off + g_sections[g_iopmod_idx].sh_size + 15) & ~15u;
    g_out_shstrtab_off = g_out_alloc_off + g_out_filesz;

    g_out_shnum = (uint32_t)nkeep + (scratch_reloc_word_count() > 0 ? 1u : 0u) + 4;
    /* NULL + kept + (.elf2irx_padlo if any) + shstrtab/symtab/strtab */
    uint32_t iopmod_size = g_sections[g_iopmod_idx].sh_size;
    (void)iopmod_size;
    g_out_shoff      = (g_out_shstrtab_off + g_out_shstr_size + 3) & ~3u;
    ...
    for (int i = 0; i < g_e_shnum; i++) {
        struct Section *s = &g_sections[i];
        if (s->keep && s->sh_type == SHT_REL) reloc_total += s->out_size;
    }
    g_out_symtab_off = (g_out_relocbase + reloc_total + 3) & ~3u;
    ...
    if (g_verbose) {
        ...
        fprintf(stderr, "  alloc_off   0x%x  (filesz 0x%x  memsz 0x%x)\n",
                g_out_alloc_off, g_out_filesz, g_out_memsz);
...
static void apply_static_relocation_addends(uint8_t *out) {
    for (int i = 0; i < g_e_shnum; i++) {
        ...
        if (!rel->keep || rel->sh_type != SHT_REL) continue;
        if (rel->sh_info >= g_e_shnum) continue;
        ...
        for (uint32_t k = 0; k < n; k++) {
            ...
            case R_MIPS_16: {
                ...
                wr32(place, (word & 0xffff0000u) | (value & 0xffffu));
                break;
            }
            case R_MIPS_32:
            case R_MIPS_REL32:
                ...
                break;
            case R_MIPS_26: {
                uint32_t value = ((word & 0x03ffffffu) << 2) + sym_addr;
                wr32(place, (word & 0xfc000000u) | ((value >> 2) & 0x03ffffffu));
                break;
            }
            case R_MIPS_HI16: {
                uint32_t lo_off = 0;
                ...
                for (uint32_t j = k + 1; j < n; j++) {
                    ...
                    if (ELF32_R_TYPE(info2) != R_MIPS_LO16) continue;
                    if (ELF32_R_SYM(info2) != ELF32_R_SYM(info)) continue;
                ...
                int32_t addend = (int32_t)((word & 0xffffu) << 16) + sign_extend16(lo_word);
                uint32_t value = (uint32_t)(addend + (int32_t)sym_addr);
                wr32(place, (word & 0xffff0000u) | (((value + 0x8000u) >> 16) & 0xffffu));
                break;
            }
            case R_MIPS_LO16: {
...
static void emit_relocation_section(const struct Section *s, uint8_t *out) {
        ...
        if (type == R_MIPS_HI16) {
            for (uint32_t i = 0; i < g_stray_count; i++) {
                if (g_strays[i].rel_sec_idx == rel_sec_idx &&
                    g_strays[i].hi16_in_idx == (uint32_t)k) {
            ...
            for (uint32_t i = 0; i < g_extra_lo_count; i++) {
                if (g_extra_los[i].rel_sec_idx == rel_sec_idx &&
                    g_extra_los[i].lo16_in_idx == (uint32_t)k) {
    ...
    if (!pair || !lo_user) die("oom for pair tables");
    for (size_t k = 0; k < nk; k++) { pair[k] = -1; lo_user[k] = -1; }
    for (size_t k = 0; k < nk; k++) {
        if (kept[k].type != R_MIPS_HI16) continue;
        if (kept[k].stray_idx >= 0) continue;
        int m = -1;
        for (size_t j = k + 1; j < nk; j++) {
            if (kept[j].type == R_MIPS_LO16 &&
                kept[j].sym  == kept[k].sym) {
                m = (int)j;
                break;
            }
        }
        ...
        if (lo_user[m] >= 0)
            die("%s: internal error — non-stray HI16 at offset 0x%x and "
                ...
                s->name, kept[(size_t)lo_user[m]].off_in,
                kept[k].off_in, kept[m].off_in);
        lo_user[m] = (int)k;
        pair[k]    = m;
    }
    ...
    if (!consumed) die("oom for consumed flags");
    size_t out_k = 0;
    for (size_t k = 0; k < nk; k++) {
        if (consumed[k]) continue;
        if (kept[k].extra_lo_idx >= 0) {
            uint32_t scratch_hi_off = g_padlo_addr + g_stray_count * 4 +
                                      (uint32_t)kept[k].extra_lo_idx * 4;
        ...
        if (kept[k].stray_idx >= 0) {
            ...
            uint32_t padlo_off = g_padlo_addr +
                                 (uint32_t)kept[k].stray_idx * 4;
        ...
        size_t next_k = k + 1;
        while (next_k < nk && consumed[next_k]) next_k++;
    ...
    size_t expected = nk + section_stray_count(rel_sec_idx) +
                      section_extra_lo_count(rel_sec_idx);
    if (out_k != expected)
        die("%s: internal error — emitted %zu reloc entries, expected %zu",
            s->name, out_k, expected);
...
static void emit_irx(void) {
    ...
    if (!out) die("oom output buffer %u bytes", g_out_total_size);
    ...
    for (int i = 0; i < g_e_shnum; i++) {
        struct Section *s = &g_sections[i];
        if (!s->keep) continue;
        if (i == g_iopmod_idx) continue;
        if (s->sh_type != SHT_PROGBITS) continue;
        if (!(s->sh_flags & SHF_ALLOC)) continue;
        memcpy(out + g_out_alloc_off + s->out_addr,
               g_in + s->sh_offset, s->sh_size);
    }
    ...
    if (scratch_reloc_word_count() > 0) {
        for (uint32_t i = 0; i < g_stray_count; i++) {
            ...
            const uint8_t *lo_instr = out + g_out_alloc_off +
                                      target_addr + sh->lo16_off_in;
        ...
        for (uint32_t i = 0; i < g_extra_lo_count; i++) {
            ...
            const uint8_t *hi_instr = out + g_out_alloc_off +
                                      target_addr + el->hi16_off_in;
    ...
    if (!f) die("cannot open output %s", g_out_name);
...
int main(int argc, char **argv) {
    ...
    while (argi < argc && argv[argi][0] == '-') {
        if (strcmp(argv[argi], "-v") == 0) { g_verbose = 1; argi++; continue; }
